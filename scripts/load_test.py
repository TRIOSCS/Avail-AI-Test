...
VENDORS = [
    "Arrow Electronics", "Digi-Key", "Mouser Electronics", "Newark",
    "Farnell", "Future Electronics", "TTI Inc", "Avnet", "RS Components",
    "Rochester Electronics", "Chip1Stop", "LCSC", "TME",
    "Heilind Electronics", "Master Electronics", "Smith & Associates",
    "Fusion Worldwide", "SourcEnginge", "Bisco Industries", "Symmetry Electronics",
]
...
def init_globals(db):
    ...
    CUSTOMER_SITES = db.execute(text(
    ...
    # Next quote number
    r = db.execute(text(
        "SELECT COALESCE(MAX(CAST(quote_number AS INTEGER)),95000) "
        "FROM quotes WHERE quote_number ~ '^[0-9]+$'"
    )).scalar()
    NEXT_QN = r + 1
...
def run_cycle(db, cycle_num):
    ...
    for user_id, user_name, user_role in SALES_USERS:
      try:
        ...
        ), {"name": req_name, "site": site_id, "cname": site[2],
            "uid": user_id, "now": now})
        ...
        for mpn, mfg, subs in parts_batch:
            ...
            r = db.execute(text(
                ...
                "rid": req_id, "mpn": mpn, "nmpn": mpn.upper().replace(" ", ""),
                "brand": mfg, "qty": target_qty, "price": target_price,
                "subs": sub_json,
        ...
        for item_id, mpn, mfg, qty, price in req_items:
            ...
            for _ in range(num_sightings):
                ...
                db.execute(text(
                    ...
                    "rid": item_id, "vn": vendor, "vnn": vendor.lower(),
                    "mpn": mpn, "nmpn": mpn.upper().replace(" ", ""),
                    "mfg": mfg, "qty": s_qty, "price": s_price,
        ...
        db.execute(text(
            "UPDATE requisitions SET status='sourcing', last_searched_at=:now WHERE id=:id"
        ), {"id": req_id, "now": now})
        db.commit()
        ...
        for item_id, mpn, mfg, qty, price in req_items:
            ...
            for vendor in vendors_used:
                ...
                r = db.execute(text(
                    ...
                    "rid": req_id, "iid": item_id,
                    "vn": vendor, "vnn": vendor.lower(),
                    "uid": user_id, "mpn": mpn,
                    "nmpn": mpn.upper().replace(" ", ""),
                    "mfg": mfg, "qty": o_qty, "price": o_price,
        ...
        db.execute(text(
            "UPDATE requisitions SET status='offers' WHERE id=:id"
        ), {"id": req_id})
        db.commit()
        ...
        for oid, mpn, vendor, qty, cost_price, item_id in quote_offers:
            ...
            line_items.append({
                "offer_id": oid, "mpn": mpn, "vendor_name": vendor,
                "qty": qty, "cost_price": cost_price, "sell_price": sell_price,
                "margin_pct": margin, "manufacturer": mfg,
            })
        ...
        r = db.execute(text(
            ...
            "rid": req_id, "sid": site_id, "qn": qn,
            "items": json.dumps(line_items),
            "sub": round(subtotal, 2), "cost": round(total_cost, 2),
            "margin": margin_pct,
        ...
        db.execute(text(
            "UPDATE quotes SET status='sent', sent_at=:now WHERE id=:id"
        ), {"id": quote_id, "now": now})

        # Update req status
        db.execute(text(
            "UPDATE requisitions SET status='quoted' WHERE id=:id"
        ), {"id": req_id})
        db.commit()
    ...
    for quote_id, qn, req_id, site_id, uid, items, subtotal, total_cost in cycle_quotes:
        ...
        reason = random.choice([
            "Price competitive", "Good lead time", "Preferred vendor",
            "Customer urgent need", "Repeat order",
        ]) if result == "won" else random.choice([
            "Price too high", "Lead time too long", "Found alternate source",
            "Customer cancelled project", "Budget cut", "Went with competitor",
        ])
        ...
        db.execute(text(
            ...
            "id": quote_id, "st": result, "res": result,
            "reason": reason, "now": now,
            "rev": round(subtotal, 2) if result == "won" else None,
        })

        db.execute(text(
            "UPDATE requisitions SET status=:st WHERE id=:id"
        ), {"id": req_id, "st": result})
    ...
    for quote_id, qn, req_id, site_id, uid, items, subtotal, total_cost in won_quotes:
        ...
        for item in items:
            bp_items.append({
                **item,
                "po_number": None, "po_entered_at": None,
                "po_sent_at": None, "po_verified": False,
                "entered_by_id": uid,
            })
        ...
        if final_status in ("approved", "po_entered", "po_confirmed", "complete"):
            db.execute(text(
                "UPDATE buy_plans SET status='approved', approved_by_id=1, approved_at=:now "
                "WHERE id=:id"
            ), {"id": bp_id, "now": now})
        ...
        if final_status in ("po_entered", "po_confirmed", "complete"):
            ...
            db.execute(text(
                "UPDATE buy_plans SET status='po_entered', line_items=:items WHERE id=:id"
            ), {"id": bp_id, "items": json.dumps(bp_items)})

        if final_status in ("po_confirmed", "complete"):
            db.execute(text(
                "UPDATE buy_plans SET status='po_confirmed' WHERE id=:id"
            ), {"id": bp_id})
            po_confirmed_plans.append(bp_id)

        if final_status == "complete":
            db.execute(text(
                "UPDATE buy_plans SET status='complete', completed_at=:now, completed_by_id=:uid WHERE id=:id"
            ), {"id": bp_id, "uid": uid, "now": now})

        if final_status == "rejected":
            db.execute(text(
                "UPDATE buy_plans SET status='rejected', rejection_reason='Test rejection' WHERE id=:id"
            ), {"id": bp_id})
    ...
    for user_id, user_name, user_role in SALES_USERS:
        for _ in range(5):
            ...
            for oid, mpn, vendor, qty, cost, _ in match_offers:
                ...
                pro_items.append({
                    "offer_id": oid, "mpn": mpn, "vendor_name": vendor,
                    "qty": qty, "cost_price": cost, "sell_price": sell,
                })
                total_sell += qty * sell
                total_cost += qty * cost
            ...
            r = db.execute(text(
                ...
                "cids": recipient_ids, "emails": recipient_emails,
    ...
    return {
        "reqs": total_reqs, "offers": len(cycle_offers),
        "quotes": len(cycle_quotes), "won": won_count,
