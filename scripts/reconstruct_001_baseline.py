...
def _checkout_baseline_models(dest_dir: Path) -> Path | None:
    ...
    The d6ffe05d snapshot was 28 tables; migrations 002+ implicitly assume today's
    ~84-table schema (because 001 used to be Base.metadata.create_all against today's
    live models). The right baseline for the explicit-DDL rewrite is therefore today's
    models, not the Feb-2026 snapshot. See _create_all_against_live_models below.
    """
    return None
