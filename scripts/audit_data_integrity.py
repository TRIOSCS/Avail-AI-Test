...
def audit(engine, fix: bool = False) -> dict:
    ...
    with engine.connect() as conn:
        ...
        rows = conn.execute(
            text("SELECT id, target_qty FROM requirements WHERE target_qty IS NOT NULL AND target_qty < 1")
        ).fetchall()
        ...
        rows = conn.execute(
            text("SELECT id, target_price FROM requirements WHERE target_price IS NOT NULL AND target_price < 0")
        ).fetchall()
        report["checks"].append({"name": "req_bad_target_price", "count": len(rows)})
        if fix and rows:
            conn.execute(
                text("UPDATE requirements SET target_price = NULL WHERE target_price IS NOT NULL AND target_price < 0")
            )
            conn.commit()
            report["fixed"] += len(rows)
        ...
        rows = conn.execute(
            text("SELECT id FROM sightings WHERE qty_available IS NOT NULL AND qty_available <= 0")
        ).fetchall()
        report["checks"].append({"name": "sight_bad_qty", "count": len(rows)})
        if fix and rows:
            conn.execute(
                text("UPDATE sightings SET qty_available = NULL WHERE qty_available IS NOT NULL AND qty_available <= 0")
            )
            conn.commit()
            report["fixed"] += len(rows)
        ...
        rows = conn.execute(
            text("SELECT id FROM sightings WHERE unit_price IS NOT NULL AND unit_price <= 0")
        ).fetchall()
        report["checks"].append({"name": "sight_bad_price", "count": len(rows)})
        if fix and rows:
            conn.execute(
                text("UPDATE sightings SET unit_price = NULL WHERE unit_price IS NOT NULL AND unit_price <= 0")
            )
            conn.commit()
            report["fixed"] += len(rows)
        ...
        rows = conn.execute(
            text("SELECT id FROM sightings WHERE confidence IS NOT NULL AND (confidence < 0 OR confidence > 1)")
        ).fetchall()
        ...
        rows = conn.execute(
            text("SELECT id FROM sightings WHERE lead_time_days IS NOT NULL AND lead_time_days < 0")
        ).fetchall()
        report["checks"].append({"name": "sight_bad_lead_time", "count": len(rows)})
        if fix and rows:
            conn.execute(
                text(
                    "UPDATE sightings SET lead_time_days = NULL WHERE lead_time_days IS NOT NULL AND lead_time_days < 0"
                )
            )
            conn.commit()
            report["fixed"] += len(rows)
        ...
        rows = conn.execute(
            text("SELECT id, status FROM offers WHERE status NOT IN ('active','expired','won','lost','pending_review')")
        ).fetchall()
