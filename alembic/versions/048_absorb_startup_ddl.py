...
def upgrade() -> None:
    ...
    _add_col("activity_log", sa.Column(
        "customer_site_id", sa.Integer(),
        sa.ForeignKey("customer_sites.id"),
    ))
    _add_col("activity_log", sa.Column(
        "site_contact_id", sa.Integer(),
    ...
    # ── Index creations (16) ──────────────────────────────────────────
    # Trigram indexes (GIN)
    _add_idx("ix_requisitions_name_trgm", "requisitions", ["name"],
             postgresql_using="gin", postgresql_ops={"name": "gin_trgm_ops"})
    _add_idx("ix_requisitions_customer_name_trgm", "requisitions", ["customer_name"],
             postgresql_using="gin", postgresql_ops={"customer_name": "gin_trgm_ops"})
    _add_idx("ix_requirements_mpn_trgm", "requirements", ["primary_mpn"],
             postgresql_using="gin", postgresql_ops={"primary_mpn": "gin_trgm_ops"})
    _add_idx("ix_requirements_subs_trgm", "requirements", ["substitutes_text"],
             postgresql_using="gin", postgresql_ops={"substitutes_text": "gin_trgm_ops"})

    # Partial / functional indexes
    _add_idx("ix_vendor_cards_blacklisted", "vendor_cards", ["is_blacklisted"],
    ...
    if not _idx_exists("requirements", "ix_requirements_primary_mpn"):
        conn.execute(sa.text(
            "CREATE INDEX ix_requirements_primary_mpn ON requirements (LOWER(primary_mpn))"
        ))

    _add_idx("ix_offers_vendor_card", "offers", ["vendor_card_id"],
             postgresql_where=sa.text("vendor_card_id IS NOT NULL"))
    _add_idx("ix_contacts_vendor_name", "contacts", ["vendor_name"],
             postgresql_where=sa.text("vendor_name IS NOT NULL"))
    _add_idx("ix_vendor_responses_vendor_name", "vendor_responses", ["vendor_name"],
             postgresql_where=sa.text("vendor_name IS NOT NULL"))
    ...
    checks = [
        ...
        ("requirements", "chk_req_condition",
         "condition IS NULL OR condition IN ('new','refurb','used')"),
        ("requirements", "chk_req_packaging",
        ...
        ("sightings", "chk_sight_confidence",
        ...
        ("sightings", "chk_sight_condition",
         "condition IS NULL OR condition IN ('new','refurb','used','other')"),
        ("sightings", "chk_sight_packaging",
        ...
        ("offers", "chk_offer_condition",
         "condition IS NULL OR condition IN ('new','refurb','used','other')"),
        ("offers", "chk_offer_packaging",
         "packaging IS NULL OR packaging IN "
         "('reel','tube','tray','bulk','cut_tape','bag','box','each','strip','other')"),
        ("offers", "chk_offer_status",
         "status IN ('active','expired','won','lost','pending_review','rejected')"),
    ]
    for table, name, check in checks:
        if not _constraint_exists(name):
            conn.execute(sa.text(
                f"ALTER TABLE {table} ADD CONSTRAINT {name} CHECK ({check}) NOT VALID"
            ))
    ...
    conn.execute(sa.text(
        "UPDATE site_contacts SET is_active = TRUE WHERE is_active IS NULL"
    ))

    # Backfill api_sources.is_active from status
    conn.execute(sa.text(
        "UPDATE api_sources SET is_active = TRUE WHERE status = 'live' AND is_active = FALSE"
    ))
...
def downgrade() -> None:
    ...
    for name in [
        ...
        "chk_offer_qty", "chk_offer_price", "chk_offer_moq", "chk_offer_condition",
        "chk_offer_packaging", "chk_offer_status",
    ]:
        conn.execute(sa.text(
    ...
    # ── Drop indexes ──────────────────────────────────────────────────
    for idx in [
        "ix_requisitions_name_trgm", "ix_requisitions_customer_name_trgm",
        "ix_requirements_mpn_trgm", "ix_requirements_subs_trgm",
        "ix_vendor_cards_blacklisted", "ix_requirements_primary_mpn",
        "ix_offers_vendor_card", "ix_contacts_vendor_name",
        "ix_vendor_responses_vendor_name", "ix_offers_vendor_name",
        "ix_vendor_cards_score_computed_at",
        "ix_activity_log_customer_site_id", "ix_activity_log_site_contact_id",
        "ix_requisitions_updated_by_id", "ix_offers_approved_by_id",
