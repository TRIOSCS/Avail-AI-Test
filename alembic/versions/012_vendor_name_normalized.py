...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE IF EXISTS contacts DROP COLUMN IF EXISTS vendor_name_normalized")
    op.execute("ALTER TABLE IF EXISTS offers DROP COLUMN IF EXISTS vendor_name_normalized")
    op.execute("ALTER TABLE IF EXISTS material_vendor_history DROP COLUMN IF EXISTS vendor_name_normalized")
