...
def upgrade() -> None:
    ...
    op.drop_column('material_cards', 'sf_material_id')
    op.drop_column('requirements', 'sf_req_item_id')
    op.drop_column('requisitions', 'sf_requisition_id')
    op.drop_column('vendor_cards', 'acctivate_vendor_id')
    op.drop_column('vendor_cards', 'acctivate_last_order_date')
    op.drop_column('vendor_cards', 'acctivate_total_units')
    op.drop_column('vendor_cards', 'acctivate_total_orders')
    op.drop_column('vendor_cards', 'sf_account_id')
    op.drop_column('vendor_cards', 'last_synced_at')
    op.drop_column('material_vendor_history', 'acctivate_last_price')
    op.drop_column('material_vendor_history', 'acctivate_rma_rate')
    op.drop_column('material_vendor_history', 'acctivate_last_date')
    ...
    op.create_index('ix_activity_external', 'activity_log', ['external_id'], unique=False, postgresql_where=sa.text('external_id IS NOT NULL'))
    op.create_index('ix_activity_site_contact', 'activity_log', ['site_contact_id', 'created_at'], unique=False, postgresql_where=sa.text('site_contact_id IS NOT NULL'))
    op.create_index('ix_activity_user_notif', 'activity_log', ['user_id', 'activity_type', 'created_at'], unique=False, postgresql_where=sa.text('dismissed_at IS NULL'))
    ...
    op.create_index('ix_sightings_req_score', 'sightings', ['requirement_id', sa.literal_column('score DESC')], unique=False)
    ...
    op.create_foreign_key('fk_activity_log_site_contact', 'activity_log', 'site_contacts', ['site_contact_id'], ['id'], ondelete='SET NULL')

    op.drop_constraint('contacts_requisition_id_fkey', 'contacts', type_='foreignkey')
    op.create_foreign_key('fk_contacts_requisition', 'contacts', 'requisitions', ['requisition_id'], ['id'], ondelete='CASCADE')

    op.drop_constraint('offers_requirement_id_fkey', 'offers', type_='foreignkey')
    op.create_foreign_key('fk_offers_requirement', 'offers', 'requirements', ['requirement_id'], ['id'], ondelete='CASCADE')

    op.drop_constraint('requirements_requisition_id_fkey', 'requirements', type_='foreignkey')
    op.create_foreign_key('fk_requirements_requisition', 'requirements', 'requisitions', ['requisition_id'], ['id'], ondelete='CASCADE')
    ...
    op.create_foreign_key('fk_requisitions_site', 'requisitions', 'customer_sites', ['customer_site_id'], ['id'], ondelete='SET NULL')
    op.create_foreign_key('fk_requisitions_created_by', 'requisitions', 'users', ['created_by'], ['id'], ondelete='SET NULL')

    op.drop_constraint('sightings_requirement_id_fkey', 'sightings', type_='foreignkey')
    op.create_foreign_key('fk_sightings_requirement', 'sightings', 'requirements', ['requirement_id'], ['id'], ondelete='CASCADE')
...
def downgrade() -> None:
    ...
    op.create_foreign_key('requisitions_customer_site_id_fkey', 'requisitions', 'customer_sites', ['customer_site_id'], ['id'])

    op.drop_constraint('fk_requirements_requisition', 'requirements', type_='foreignkey')
    op.create_foreign_key('requirements_requisition_id_fkey', 'requirements', 'requisitions', ['requisition_id'], ['id'])
    ...
    op.create_foreign_key('activity_log_site_contact_id_fkey', 'activity_log', 'site_contacts', ['site_contact_id'], ['id'])
