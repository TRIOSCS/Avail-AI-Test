...
class Citation(BaseModel):
    ...
    def _validate_http_scheme(cls, v: str) -> str:
        # Parse the scheme structurally rather than with ``startswith`` so
        # leading-whitespace tricks (e.g. " javascript:alert(1)") and odd
        # casing can't slip a dangerous scheme past a prefix check.
...
class ResolverLlmResponse(BaseModel):
    """Schema for what the LLM must return when resolving a spec code.

    The OUTER model uses ``extra="ignore"`` so that a stray top-level key from
    the LLM (e.g. a ``notes`` field it wasn't asked for) does not turn an
    otherwise-valid resolution into ``unresolved``. The nested ``AvlEntry`` and
    ``Citation`` models keep ``extra="forbid"`` — their shapes are
    safety-critical (MPNs we source against, URLs we may render).
    """
