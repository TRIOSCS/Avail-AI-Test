...
def _ladder_set(
    ...
    *,
    ...
    prior = overlay.get(key)  # type: ignore[arg-type, unused-ignore]  # legacy Column-model ORM noise
    if prior is None:
    ...
    if won:
        overlay[key] = {**incoming, "value": normalize_brand_name(db, str(value))}  # type: ignore[index, unused-ignore]  # legacy Column-model ORM noise
    return won
    ...
    for card in cards:
        ...
        try:
            won = _ladder_set(
                ...
                card,
                "brand",
                card.manufacturer,  # type: ignore[arg-type, unused-ignore]  # legacy Column-model ORM noise
                "legacy_backfill",
                0.5,
                apply=apply,
                overlay=overlay,
...
def _final_state(db: Session, card: MaterialCard, attr: str, *, apply: bool, overlay: _Overlay) -> tuple:
    ...
    if not apply:
        entry = overlay.get((card.id, attr))  # type: ignore[arg-type, unused-ignore]  # legacy Column-model ORM noise
