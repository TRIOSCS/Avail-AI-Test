...
def _get_or_create_list(
    ...
    if close_in_days is not None and hasattr(el, "close_at"):
        el.close_at = _now() + timedelta(days=close_in_days)  # type: ignore[assignment, unused-ignore]  # legacy Column-model ORM noise
    db.add(el)
    db.flush()
...
def _build_collecting(db: Session, company: Company, owner: User, broker: User) -> None:
    ...
    if created:
        ...
        el.total_line_items = len(_BULK_MPNS)  # type: ignore[assignment, unused-ignore]  # legacy Column-model ORM noise
        db.commit()
    ...
    excess_service.submit_offer(
        db,
        list_id=el.id,  # type: ignore[arg-type, unused-ignore]  # legacy Column-model ORM noise
        user=broker,
...
def _build_oneoff(db: Session, company: Company, owner: User, broker: User) -> None:
    ...
    if created:
        ...
        el.total_line_items = 1  # type: ignore[assignment, unused-ignore]  # legacy Column-model ORM noise
        db.commit()
        excess_mirror.sync_list_mirror(db, el)
        db.commit()
...
def _build_awarded(db: Session, company: Company, owner: User) -> None:
    ...
    if created:
        for mpn, mfr in _BULK_MPNS[:6]:
            ...
            item.status = ExcessLineItemStatus.AWARDED  # type: ignore[assignment, unused-ignore]  # legacy Column-model ORM noise
        el.total_line_items = 6  # type: ignore[assignment, unused-ignore]  # legacy Column-model ORM noise
        db.commit()
