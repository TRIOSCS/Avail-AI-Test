...
class Company(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_companies_primary_contact_id", "primary_contact_id"),
...
class SiteContact(Base):
    ...
    reports_to_id = Column(Integer, ForeignKey("site_contacts.id", ondelete="SET NULL"), nullable=True)
    ...
    __table_args__ = (
        ...
        # Raw-DDL indexes reconciled into the model so the drift gate sees them (#464).
        # ix_sc_reports_to (migration 144) replaces the column-level index=True (which
        # autogenerated the never-created name ix_site_contacts_reports_to_id).
        Index("ix_sc_reports_to", "reports_to_id"),
        Index("ix_site_contacts_email_trgm", "email", postgresql_using="gin", postgresql_ops={"email": "gin_trgm_ops"}),
        Index(
