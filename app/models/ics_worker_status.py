...
- API server reads it for /api/ics/worker/health endpoint

Called by: ics_worker.worker (heartbeat updates), ics_admin router (reads)
...
class IcsWorkerStatus(Base):
    ...
    __table_args__ = (
        CheckConstraint("id = 1", name="ck_ics_worker_status_singleton"),
    )
