...
@router.get("/api/enrichment/clay/authorize")
def api_clay_authorize(user: User = Depends(require_admin)):
    """Build Clay OAuth2 authorization URL with PKCE. Returns URL for admin to open."""
    from ..connectors.clay_client import build_authorize_url
...
@router.get("/api/clay/callback")
async def api_clay_callback(
    code: str = Query(...),
    state: str = Query(...),
    db: Session = Depends(get_db),
):
    """OAuth2 callback — exchanges auth code for tokens and stores them."""
    from ..connectors.clay_client import exchange_code_for_tokens, _save_token_to_db

    token_data = await exchange_code_for_tokens(code, state)
    if not token_data or "access_token" not in token_data:
        raise HTTPException(400, "Failed to exchange authorization code for tokens")

    _save_token_to_db(db, token_data)
    db.commit()
    logger.info("Clay OAuth tokens stored successfully")
...
@router.get("/api/enrichment/clay/status")
def api_clay_status(
    user: User = Depends(require_user),
    db: Session = Depends(get_db),
):
    """Check whether Clay OAuth tokens are stored and valid."""
    from ..models.enrichment import ClayOAuthToken

    token = db.query(ClayOAuthToken).first()
    if not token:
        return {"connected": False, "message": "Not connected"}

    expired = token.expires_at < datetime.now(timezone.utc)
    return {
        "connected": True,
        "expired": expired,
