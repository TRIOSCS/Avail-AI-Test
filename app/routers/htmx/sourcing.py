...
def _lead_row_sighting(db: Session, lead) -> dict[int, dict]:
    """Latest-sighting ``{qty_available, unit_price}`` map for ONE lead.

    Single-lead companion to ``_lead_sighting_data`` used by lead_status_update's
    row/card re-renders (the response carries exactly one lead).
    """
    best_sighting = (
        db.query(Sighting)
        .filter(
...
def _filtered_leads_ctx(
    ...
    *,
    ...
    """Shared filter/sort/paginate/context assembly for the lead-list surfaces.

    One implementation behind sourcing_results_partial, sourcing_workspace_partial,
    and sourcing_workspace_list_partial so the results grid, workspace, and
    workspace list can never diverge on the same query string. Raises 404 for an
    unknown requirement; returns the full template ctx (requirement, page of leads,
    sighting data, paging, ``f_*`` filter echoes) — each caller adds its own
    ``selected_lead_id`` / template choice.
    """
...
def _lead_detail_ctx(request: Request, user: User, db: Session, lead_id: int) -> dict:
    """Shared lead-detail context: evidence (by confidence impact), category grouping
    + labels, requirement, vendor card, and best sighting.

    One implementation behind lead_detail_partial (full page) and lead_panel_partial
    (workspace right panel) so the evidence grouping and category vocabulary cannot
...
async def lead_status_update(
    ...
    if hx_target.startswith("lead-row-"):
        ctx = _base_ctx(request, user, "requisitions")
        ctx.update({"lead": lead, "lead_sighting_data": _lead_row_sighting(db, lead), "selected_lead_id": 0})
        return template_response("htmx/partials/sourcing/lead_row.html", ctx)

    # Default: card view (results grid)
    ctx = _base_ctx(request, user, "requisitions")
    ctx.update({"lead": lead, "lead_sighting_data": _lead_row_sighting(db, lead)})
...
async def sourcing_workspace_partial(
    ...
    ctx["selected_lead_id"] = lead if lead else 0
