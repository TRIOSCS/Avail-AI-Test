...
async def _deliver_follow_up(request, db, contact, user, *, token, is_testing, body=""):
...
async def send_follow_up_htmx(
    ...
    result = await _deliver_follow_up(request, db, contact, user, token=None, is_testing=is_testing, body=body)
    if result == "sent":
        db.commit()
...
async def send_batch_follow_up(
    ...
    for contact in stale:
        result = await _deliver_follow_up(request, db, contact, user, token=token, is_testing=is_testing)
    ...
    toast_type = (
        ...
        else ("warning" if (tally["failed"] or skipped) else "success")
    )
