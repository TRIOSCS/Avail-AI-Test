...
from ..services.vendor_coverage_service import _vss_vendor_card_join
from ..services.vendor_coverage_service import coverage_ranked_vendor_rows as _coverage_ranked_vendor_rows
...
# _vss_vendor_card_join / RankedVendor / coverage_ranked_vendor_rows moved to
# app.services.vendor_coverage_service (item 19) — 141 lines of pure business logic
# with no HTTP concerns. Imported at the top of this file (aliased back to
# _coverage_ranked_vendor_rows) so this router's two call sites, and the existing
# test suite (which imports/calls it off ``app.routers.sightings``), keep working
# unmodified.
...
async def sightings_review_offer(
    ...
    if offer is None or offer.requirement_id != requirement_id:
        raise HTTPException(status_code=404, detail="offer not found for this requirement")
    if action == "approve":
...
async def sightings_offer_request(
    ...
    if kind not in REQUEST_KINDS:
        raise HTTPException(status_code=400, detail="invalid request kind")
    offer = db.get(Offer, offer_id)
    # Scope the offer to the path requirement (prevents cross-requirement IDOR via a
