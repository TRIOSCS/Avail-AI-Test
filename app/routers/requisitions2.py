...
async def requisition_detail_panel(
    request: Request,
    req_id: int,
    db: Session = Depends(get_db),
    user: User = Depends(require_user),
):
    """Detail panel content for HTMX swap into #rq2-detail (right panel)."""
...
# ── Detail modal (legacy — kept for backward compat) ────────────────
...
async def requisition_modal(
    ...
    """Detail modal content — legacy endpoint, kept for backward compat."""
    detail = get_requisition_detail(
        db=db,
        req_id=req_id,
        user_id=user.id,
