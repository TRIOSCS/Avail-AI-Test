...
async def list_requisitions(q: str = "", user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    return [{
        ...
        "customer_display": (
            ...
            if r.customer_site and r.customer_site.company else r.customer_name or ""
        ),
...
async def create_requisition(body: RequisitionCreate, user: User = Depends(require_user), db: Session = Depends(get_db)):
    req = Requisition(
        name=body.name,
...
async def list_requirements(req_id: int, user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    if req.requirements:
        ...
        rows = db.query(
            ...
            sqlfunc.count(sqlfunc.distinct(sqlfunc.lower(sqlfunc.trim(Sighting.vendor_name))))
        ).filter(
            Sighting.requirement_id.in_(req_ids),
            Sighting.vendor_name.isnot(None),
        ).group_by(Sighting.requirement_id).all()
        for rid, cnt in rows:
...
async def add_requirements(req_id: int, request: Request, user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    for item in items:
        ...
        r = Requirement(requisition_id=req_id, primary_mpn=parsed.primary_mpn,
...
async def update_requirement(item_id: int, data: RequirementUpdate, user: User = Depends(require_user), db: Session = Depends(get_db)):
    r = db.get(Requirement, item_id)
    if not r:
        raise HTTPException(404)
    req = get_req_for_user(db, user, r.requisition_id)
...
async def mark_unavailable(sighting_id: int, data: SightingUnavailableIn,
                           user: User = Depends(require_user), db: Session = Depends(get_db)):
    s = db.get(Sighting, sighting_id)
    if not s:
        raise HTTPException(404)
    # Verify ownership: sighting → requirement → requisition → user (sales restricted)
    req_check = db.query(Requisition).join(Requirement).filter(
...
async def import_stock_list(req_id: int, request: Request,
    ...
    for row in rows:
        mpn = (row.get("mpn") or row.get("part_number") or row.get("part")
               or row.get("pn") or row.get("sku") or row.get("mfr_part") or "").strip()
        if not mpn:
            continue
        imported += 1
        ...
        qty_str = row.get("qty") or row.get("quantity") or row.get("qty_available") or ""
