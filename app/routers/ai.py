...
# ── Feature 5a: Intake draft (universal text classifier) ─────────────────
...
async def ai_intake_draft(
    ...
    """Classify pasted text and return structured RFQ or offer draft."""
    if not _ai_enabled(user):
        raise HTTPException(403, "AI features not enabled")
    ...
    result = await parse_freeform_intake(payload.text, req_context)
    if not result:
        return {"document_type": "unclear", "confidence": 0, "requirements": [], "offers": []}
    return result
