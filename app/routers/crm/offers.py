...
async def list_offers(req_id: int, user: User = Depends(require_user), db: Session = Depends(get_db)):
    ...
    risk_flags_map: dict[int, list[dict]] = {}
    if offers:
        ...
        for rf in flags:
            risk_flags_map.setdefault(rf.source_offer_id, []).append(
                {
                    "id": rf.id,
                    "type": rf.type,
        ...
        groups[key].append(
            {
                ...
                "risk_flags": risk_flags_map.get(o.id, []),
            }
        )
...
async def create_offer(
    ...
    if not req:
        raise HTTPException(404, "Requisition not found")
    vendor_name = sanitize_text(payload.vendor_name) or payload.vendor_name
    mpn = sanitize_text(payload.mpn) or payload.mpn
    ...
    vendor_website = sanitize_text(payload.vendor_website) if payload.vendor_website else payload.vendor_website
...
async def update_offer(
    ...
    if "status" in changes and changes.get("status") is not None:
        try:
    ...
    for text_field in (
        ...
        if text_field in changes and changes[text_field] is not None:
            changes[text_field] = sanitize_text(changes[text_field])
