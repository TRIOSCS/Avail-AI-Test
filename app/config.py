...
class Settings:
    ...
    do_gradient_model: str = os.getenv("DO_GRADIENT_MODEL", "anthropic-claude-sonnet-4-6")
    ...
    explorium_api_base_url: str = os.getenv("EXPLORIUM_API_BASE_URL", "https://api.explorium.ai")
    ...
    customer_enrichment_cooldown_days: int = int(os.getenv("CUSTOMER_ENRICHMENT_COOLDOWN_DAYS", "90"))
    customer_enrichment_contacts_per_account: int = int(os.getenv("CUSTOMER_ENRICHMENT_CONTACTS_PER_ACCOUNT", "5"))

    # Deep Enrichment feature flags
    deep_enrichment_enabled: bool = os.getenv("DEEP_ENRICHMENT_ENABLED", "false").lower() == "true"
    deep_email_mining_enabled: bool = os.getenv("DEEP_EMAIL_MINING_ENABLED", "false").lower() == "true"
    deep_enrichment_auto_apply_threshold: float = float(os.getenv("DEEP_ENRICHMENT_AUTO_APPLY_THRESHOLD", "0.8"))
    deep_enrichment_review_threshold: float = float(os.getenv("DEEP_ENRICHMENT_REVIEW_THRESHOLD", "0.5"))
    deep_enrichment_stale_days: int = int(os.getenv("DEEP_ENRICHMENT_STALE_DAYS", "30"))
    ...
    stock_sale_vendor_names: list = [
        n.strip().lower()
        for n in os.getenv("STOCK_SALE_VENDOR_NAMES", "trio,trio supply chain,stock,internal").split(",")
        if n.strip()
    ]
    stock_sale_notify_emails: list = [
        e.strip().lower()
        for e in os.getenv("STOCK_SALE_NOTIFY_EMAILS", "logistics@trioscs.com,accounting@trioscs.com").split(",")
        if e.strip()
    ]
    ...
    buyplan_auto_approve_threshold: float = float(os.getenv("BUYPLAN_AUTO_APPROVE_THRESHOLD", "5000"))
    ...
    buyplan_favoritism_threshold_pct: float = float(os.getenv("BUYPLAN_FAVORITISM_THRESHOLD_PCT", "60"))
    buyplan_better_offer_pct: float = float(os.getenv("BUYPLAN_BETTER_OFFER_PCT", "5"))
    ...
    contact_scoring_enabled: bool = os.getenv("CONTACT_SCORING_ENABLED", "true").lower() == "true"
    ...
    prospecting_expire_days: int = int(os.getenv("PROSPECTING_EXPIRE_DAYS", "90"))
    prospecting_resurface_days: int = int(os.getenv("PROSPECTING_RESURFACE_DAYS", "180"))
