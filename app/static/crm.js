...
async function _renderCustDrawerOverview(companyId) {
    ...
    if (!c) {
        try { c = await apiFetch('/api/companies/' + companyId); if (c) crmCustomers.push(c); } catch(_e) {}
    }
    if (!c) { body.textContent = 'Account not found'; return; }
...
function renderQuote() {
    ...
    <table class="tbl quote-table" style="font-size:11px">
        <thead><tr><th>MPN</th><th>Mfr</th><th>Qty</th><th>Cost</th><th>Target</th><th>Sell</th><th>Margin</th><th>Lead</th><th>Cond</th><th>DC</th><th title="Firmware">FW</th><th title="Hardware">HW</th></tr></thead>
        <tbody>${lines}</tbody>
    </table>
    </div>
...
// ── Buy Plan (V1 removed — all buy plan UI is now in HTMX /v2/buy-plans) ──

function openBuyPlanModal() {
    // V1 removed — redirect to HTMX buy plan view
    if (!crmQuote) return;
    showToast('Use the Buy Plans page to manage buy plans', 'info');
}
...
async function showBuyPlans() {
    // Redirect to HTMX buy plans page
    window.location.href = '/v2/buy-plans';
}
...
function setBpFilter() { /* V1 removed — use HTMX /v2/buy-plans */ }
function toggleBpMyOnly() { /* V1 removed */ }
function sortBpList() { /* V1 removed */ }
...
function renderApiHealthDashboard(container, sources) {
    // Deduplicate sources by display_name to prevent duplicate connector cards (e.g. Newark)
    const seen = new Set();
    sources = sources.filter(s => {
        const key = (s.display_name || s.name || '').toLowerCase();
...
Object.assign(window, {
    ...
    filterSiteTypeahead,
    loadCustomers, onSqContactChange,
