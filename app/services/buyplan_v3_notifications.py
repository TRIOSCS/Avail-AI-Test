...
async def notify_v3_submitted(plan: BuyPlanV3, db: Session):
    ...
    await asyncio.gather(*[_send_email(m, f"[AVAIL] Buy Plan Approval — {ctx['customer_name']}", html_body, db) for m in managers])

    # In-app
    for m in managers:
        db.add(ActivityLog(
            user_id=m.id, activity_type="buyplan_pending", channel="system",
...
async def notify_v3_approved(plan: BuyPlanV3, db: Session):
    ...
    for buyer in buyers:
        ...
        db.add(ActivityLog(
            user_id=buyer.id, activity_type="buyplan_approved", channel="system",
            requisition_id=plan.requisition_id,
    ...
    if ctx["submitter"]:
        db.add(ActivityLog(
            user_id=ctx["submitter"].id, activity_type="buyplan_approved", channel="system",
            requisition_id=plan.requisition_id,
...
async def notify_v3_rejected(plan: BuyPlanV3, db: Session):
    ...
    db.add(ActivityLog(
        user_id=ctx["submitter"].id, activity_type="buyplan_rejected", channel="system",
        requisition_id=plan.requisition_id,
        subject=f"Buy plan #{plan.id} rejected — {approver_name}",
    ))
    db.commit()

    await _teams_dm(ctx["submitter"], f"Buy Plan #{plan.id} was rejected: {plan.approval_notes or 'No reason given'}", db)
...
async def notify_v3_issue_flagged(plan: BuyPlanV3, db: Session, line_id: int, issue_type: str):
    ...
    issue_labels = {"sold_out": "Sold Out", "price_changed": "Price Changed",
                    "lead_time_changed": "Lead Time Changed", "other": "Other"}
...
async def notify_v3_po_confirmed(plan: BuyPlanV3, db: Session, line_id: int):
    ...
    ops_members = (
        ...
        .filter(VerificationGroupMember.is_active.is_(True))
        .all()
    )
    for m in ops_members:
        db.add(ActivityLog(
            user_id=m.user_id, activity_type="buyplan_pending", channel="system",
            requisition_id=plan.requisition_id,
...
async def notify_v3_completed(plan: BuyPlanV3, db: Session):
    ...
    db.add(ActivityLog(
        user_id=ctx["submitter"].id, activity_type="buyplan_completed", channel="system",
        requisition_id=plan.requisition_id,
