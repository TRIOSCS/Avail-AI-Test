...
def scan_new_offers_for_matches(db: Session) -> dict:
    ...
    new_offers = db.query(Offer).filter(
    ...
    for offer in new_offers:
        ...
        candidates = db.query(Requirement, Requisition).join(
            Requisition, Requirement.requisition_id == Requisition.id
        ).filter(
        ...
        for req_item, requisition in candidates:
            ...
            # Check throttle
            throttled = db.query(ProactiveThrottle).filter(
            ...
            # Check dedup
            existing = db.query(ProactiveMatch).filter(
...
def get_match_count(db: Session, user_id: int) -> int:
    ...
    return db.query(ProactiveMatch).filter(
        ProactiveMatch.salesperson_id == user_id,
...
async def send_proactive_offer(
    ...
    match_ids: list[int], contact_ids: list[int],
    sell_prices: dict, subject: str | None = None, notes: str | None = None,
) -> dict:
    """Send a proactive offer email to a customer. Returns the created ProactiveOffer dict."""
    # Load and validate matches
    matches = db.query(ProactiveMatch).filter(
    ...
    greeting = f"Hi {contacts[0].full_name}," if len(contacts) == 1 and contacts[0].full_name else f"Hi {contact_names}," if contact_names else "Hello,"
    ...
    try:
        ...
        result = await gc.post_json("/me/sendMail", {
            "message": {
    ...
    now = datetime.now(timezone.utc)
    for m in matches:
        existing_throttle = db.query(ProactiveThrottle).filter(
...
def get_scorecard(db: Session, salesperson_id: int | None = None) -> dict:
    ...
    total_revenue = sum(float(o.total_sell or 0) for o in all_offers)
...
def get_sent_offers(db: Session, user_id: int) -> list[dict]:
    ...
    offers = db.query(ProactiveOffer).filter(
        ProactiveOffer.salesperson_id == user_id,
    ).order_by(ProactiveOffer.sent_at.desc()).all()
