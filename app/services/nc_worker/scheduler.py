"""Search scheduler — thin NC shim over search_worker_base.

The shared implementation lives in
app.services.search_worker_base.scheduler.SearchScheduler; this subclass
binds the "NC" config-attribute prefix so existing imports and patch
targets (app.services.nc_worker.scheduler.SearchScheduler) keep working.
...
class SearchScheduler(_BaseSearchScheduler):
    """NC search scheduler over the shared base implementation."""

    def __init__(self, config: NcConfig):
        super().__init__(config, prefix="NC")
