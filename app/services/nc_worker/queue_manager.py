"""NetComponents search queue manager.

Handles enqueuing parts for NC search, deduplication, polling the
next item to search, and updating queue item status.

Called by: requisition triggers, ai_gate, worker loop, admin endpoints
Depends on: nc_search_queue model, sightings model, mpn_normalizer
"""
...
def enqueue_for_nc_search(requirement_id: int, db: Session) -> NcSearchQueue | None:
    ...
    links existing NC sightings to this requirement's material card. Returns the queue
    item or None if deduped/skipped.
    """
    req = db.get(Requirement, requirement_id)
    if not req or not req.primary_mpn:
        logger.debug("NC enqueue skip: requirement {} has no MPN", requirement_id)
        return None
    ...
    existing = db.query(NcSearchQueue).filter_by(requirement_id=requirement_id).first()
    if existing:
        logger.debug("NC enqueue skip: requirement {} already queued (id={})", requirement_id, existing.id)
        return existing

    # Dedup: look for completed searches of same normalized MPN within window
    cutoff = datetime.now(timezone.utc) - timedelta(days=_config.NC_DEDUP_WINDOW_DAYS)
    recent = (
        db.query(NcSearchQueue)
        .filter(
            NcSearchQueue.normalized_mpn == norm_mpn,
            NcSearchQueue.status == "completed",
            NcSearchQueue.last_searched_at >= cutoff,
        )
        .first()
    )

    if recent:
        # Link existing NC sightings from the previous search to this requirement's material card
        if req.material_card_id and recent.requirement_id:
            ...
            for s in nc_sightings:
                # Create a linked sighting for this requirement
                new_s = Sighting(
            ...
            db.commit()
            logger.info(
                "NC dedup: requirement {} linked {} sightings from previous search (queue {})",
                requirement_id,
                len(nc_sightings),
                recent.id,
            )
        else:
            logger.info("NC dedup: requirement {} matched recent search but no material card to link", requirement_id)
        return None
    ...
    db.commit()
    db.refresh(item)
    logger.info("NC enqueue: requirement {} queued as item {} (mpn={})", requirement_id, item.id, norm_mpn)
    return item
...
def get_next_queued_item(db: Session) -> NcSearchQueue | None:
    ...
    return (
        db.query(NcSearchQueue)
        .filter(NcSearchQueue.status == "queued")
...
def mark_status(db: Session, queue_item: NcSearchQueue, new_status: str, error: str | None = None):
    ...
    db.commit()
    logger.debug("NC queue {} status -> {}", queue_item.id, new_status)
...
def mark_completed(db: Session, queue_item: NcSearchQueue, results_found: int, sightings_created: int):
    ...
    db.commit()
    logger.info(
        "NC queue {} completed: {} results, {} sightings",
        queue_item.id,
        results_found,
        sightings_created,
    )
...
def get_queue_stats(db: Session) -> dict:
    ...
    total_today = (
        ...
        .filter(
            NcSearchQueue.status == "completed",
            NcSearchQueue.last_searched_at >= today_start,
        )
        .scalar()
    ) or 0
