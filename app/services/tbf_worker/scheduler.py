...
Controls the pacing of TBF searches to mimic human behavior:
...
class SearchScheduler:
    ...
    def next_delay(self) -> float:
        ...
        Most delays cluster around the typical_delay (300s / 5min). Occasional longer
        pauses simulate checking email, coffee, etc.
        """
        mu = math.log(self.config.TBF_TYPICAL_DELAY_SECONDS)
        sigma = 0.4
        delay = random.lognormvariate(mu, sigma)
        delay = max(self.config.TBF_MIN_DELAY_SECONDS, min(self.config.TBF_MAX_DELAY_SECONDS, delay))
