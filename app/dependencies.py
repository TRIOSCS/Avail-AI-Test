...
# ── QP section review rights — REMOVED (W3.7). The Mark-Reviewed ceremony and its
# can_review_qp_sales_section / can_review_qp_purchasing_section predicates are gone;
# section editability now rides the single stage-driven lock matrix
# (services/qp_workspace.can_edit_qp_section). The per-user grant columns
# User.can_approve_qp_sales / can_approve_qp_purchasing stay in the model/DB untouched
# (vestigial, like BuyPlan.auto_approved) — the approvals routing engine still reads
# them for the QP_SALES / QP_PURCHASING gates should a routed use ever return.
