...
- **Manufacturer** — `MaterialCard.manufacturer (indexed)` · Searchable checkbox facet (multi-select, OR-within). Top ~20 by card count via get_manufacturer_options(commodity), with a type-ahead search box for the long tail. Counts shown per manufacturer. Lives as a global facet ABOVE the commodity-specific sub_filters so it works even with no commodity selected.
  - _Manufacturer is the single most common buyer narrowing dimension (cross/alternate sourcing, AVL compliance) and applies to every part regardless of commodity. get_manufacturer_options() already returns counts and is commodity-scopable; search_materials_faceted() already accepts manufacturers=[...] via the sub_filters 'manufacturers' key the router pops out._
