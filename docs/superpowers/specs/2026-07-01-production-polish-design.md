...
4. Live-verify: seed-admin login on real PG, drive key pages (Buy Plans, Requisitions, Search, CRM, Approvals, Prospecting, Resell, Settings), CSRF-aware POSTs, workers active, Tailwind classes in built CSS, no concurrent-deploy staleness (verify container build SHA).
