...
`102_vendor_part_unavailability` (renumbered 097->101->102 as concurrent chains landed on main; round-tripped PG + SQLite, single head); model +
enum tests in `tests/test_vendor_unavailability.py`.

> Task 2b retrofits this model with the policy + provenance columns (migration 103, renumbered from 098).
...
### Task 2b: Silent-failure hardening + temporal policy (service + model + migration 098, since renumbered to 103)
...
- Create: migration **098** (since renumbered to **103** at merge-with-main) via autogenerate — ONLY these four columns, revision id ≤32 chars, downgrade drops them, `alembic heads` → single head; no backfill (NULL `qty_at_mark` ⇒ O2 never fires for legacy records — fail-closed)
