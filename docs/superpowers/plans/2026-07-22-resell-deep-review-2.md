...
**FIXED:** `expire_overdue_lists`'s sweep query now excludes (NOT EXISTS) any list holding an AWARDED line or a WON offer — such lists are never loaded, stay COLLECTING, and the skip is logged at INFO with the list id(s); no status flip invented. See `app/services/excess_service.py` (`expire_overdue_lists`); tests in `tests/test_resell_list_lifecycle.py` (`test_expire_skips_partially_awarded_list_and_award_still_works_after`, `test_expire_unawarded_overdue_list_still_expires`, `test_expire_fully_awarded_list_already_outside_sweep`).
...
**FIXED:** `_find_sent_message` now raises `email_service.DeliveryCheckUnavailable` when at least one lookup attempt errored (still returns `None` only on an all-clean genuine no-match); `retry_outreach_send`'s existing INTERRUPTED/no-resend branch now catches that exception specifically and is genuinely reachable; the other three callers (`send_batch_rfq`'s batch lookup, `_finalize_outreach_send`'s post-send id capture, `quote_send.send_quote_email`) each handle the new exception deliberately (best-effort, degrade-not-block, since their sends already succeeded). See `app/email_service.py` (`DeliveryCheckUnavailable`, `_find_sent_message`), `app/services/resell_outreach_service.py` (`retry_outreach_send`, `_finalize_outreach_send`), `app/services/quote_send.py` (`send_quote_email`); tests in `tests/test_email_service.py` (`test_exception_on_every_attempt_raises_delivery_check_unavailable`, `test_clean_no_match_still_returns_none_not_raise`, `test_partial_error_then_clean_miss_still_raises`), `tests/test_resell_outreach_async.py` (`test_retry_lookup_error_leaves_interrupted_and_never_resends`, `test_retry_graph_outage_on_every_attempt_never_resends_end_to_end`, `test_retry_reconciles_already_delivered_and_does_not_resend`, `test_retry_resends_when_not_in_sent_folder`).
