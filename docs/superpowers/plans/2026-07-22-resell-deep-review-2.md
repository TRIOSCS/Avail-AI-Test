...
**Resolution: FIXED (root-caused the ordering, per the alternative the finding names).** `send_bid_back` now flips the in-memory `status` to `sent` (and stamps `sent_at`) BEFORE rendering the PDF attachment — `generate_bid_report_pdf` re-fetches the bid via `db.get`, which returns the SAME identity-mapped, already-mutated object, so the render sees `sent` with no extra flush. The flip stays uncommitted until the send is CONFIRMED: any failure (PDF render exception or a non-`sent` send result) now explicitly rolls the session back before raising, so a failed send always leaves the bid a genuine `draft` in the database — never a false `sent` stamp. See `app/services/bid_back_service.py::send_bid_back`; tests in `tests/test_resell_bid_lifecycle.py` (`test_send_bid_back_pdf_renders_with_sent_status_not_draft`, `test_send_bid_back_pdf_render_failure_leaves_bid_draft`, plus the pre-existing `test_send_bid_back_failed_send_502`).
