...
**2026-07-15 reconciliation:** ideas **C** and **O** have since shipped too — C @0c2d2bc3
(deterministic factor-breakdown + price-sparkline hover layer, shared
`htmx/partials/shared/_score_hover.html` macros) and O @4f709c1c (Claim/Dismiss-only pool
+ manager-only Assign, reclaim/reassign retired). Nothing from the 2026-07-04 idea wave
remains open except **Q** (Tailwind v4 migration).
...
| R | **Category taxonomy — 17 unmappable non-canonical strings (28 cards) need owner mapping calls** | Migration 189 (2026-07-15 residue backfill) recovered 111 of the 139 category-stranded material cards (startup `_warn_non_canonical_categories`); the remaining **28 cards sit in 17 strings with NO unambiguous canonical bucket**, deliberately left unmapped (an alias must hold in EVERY taxonomy — these need product decisions, not guesses): bare manufacturer names `intel` (4), `nexperia` (2), `onsemi` (1), `chemi-con` (1), `infinite electronics` (1), `olimex ltd.` (1), `phoenix contact` (1), `schneider electric` (1); vendor storefront/collection buckets `adafruit accessories` (2), `industrial automation and controls` (3); ambiguous component families `circuit breakers` (2), `circuit protection` (2), `discrete semiconductor products` (2), `eeprom` (2), `isolators` (1), `varistors` (1); FRU accessory `storage controller accessory / raid cache backup power (fbwc capacitor pack)` (1). Owner calls needed per string: pick a canonical `COMMODITY_TREE` key (possibly new keys, e.g. eeprom under Memory), bucket to `other`, or clear the category. Each decision then = CATEGORY_ALIASES entry (if generalizable) + follow-up data migration, or a one-off targeted rewrite. | Captured — needs product decisions |
...
| **Sightings page — hide won/lost/archived deals** (quick fix) — ✅ Shipped @3d37a796 | The sightings page is for buyers to ACTIVELY source + find offers for OPEN requirements. Won/lost/archived deals (closed requirements) must NOT appear. Filter them out of the sightings query. Done: `_EXCLUDED_REQ_STATUSES` (WON/LOST/CANCELLED) + `_EXCLUDED_SOURCING_STATUSES` (WON/LOST/ARCHIVED) applied to the board query (`app/routers/sightings.py:95-126,383-384`). |
