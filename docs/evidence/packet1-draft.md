...
|---|---|---|
| Routes | 809 (753 unique paths) | ~804 (6 badge pollers → 1 endpoint) |
...
| Status values, all entities | 114 | 114 (dead-status removal lands tonight) |
| Test files | 1,113 | 1,107 |
| E2E spec files | 12 | 12 |
| Nav tabs | 10 + Settings | 10 (nav shrink is Wave 2) |

A final runtime route re-count happens when tonight's items land, so
the ~804 firms up in the completed packet.
...
[PENDING SLOT: 48h log summary — filled Thursday]
...
[PENDING SLOT: tonight's W1.9/W1.10/W1.13/W1.17/W1.18 results]
