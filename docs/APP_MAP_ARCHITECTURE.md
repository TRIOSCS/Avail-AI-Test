...
- `app/routers/htmx/buy_plans.py` — **deal/sourcing-cluster split (Buy Plans / Approvals slice)**:
  ...
  `_render_supervise_body`, `_render_my_queue_body` (the My Queue surface body, shared by the
  `my_queue` lens dispatch + the approve/verify-po `origin=my_queue` returns), and the
  `_APPROVALS_TABS`/`_TAB_APPROVE_ATTR`/`_PO_CUTTER_ROLES` constants. Imports `_is_ops_member` from `_shared` (a quotes route in the monolith still uses
