...
- `app/routers/htmx/offers/` — **deal/sourcing-cluster split (offer/RFQ/follow-up slice)**,
  ...
  (`/v2/partials/offers/*`) and quote-from-offers), `follow_ups.py` (follow-up queue
  ...
  activity/phone-call logging). (`rfq.py` — the requisition-scoped RFQ compose/send + AI
...
| Module | Purpose |
|--------|---------|
| `app/services/requirement_service.py` | The ONE requirement-creation pipeline (W3, spec §9): every surface that creates `Requirement` rows routes through `create_requirements()` (sync core; flushes, caller commits) or `create_requirements_ui()` (async facade adding the datasheet-capture enqueue). Carries MPN display + canonical-key normalization (`normalized_mpn` — migration 206 remapped legacy rows), condition/packaging vocabulary normalization (unmapped spellings store NULL), qty/price coercion, substitute parsing + dedup, savepoint-guarded MaterialCard resolve, tag propagation, `Source {mpn}` task auto-gen (skipped for scratch reqs), and 30-day same-customer-site duplicate detection (informational, never blocks). Called by `routers/htmx/requisitions.py` (import-save, add_requirement), `routers/htmx/search_views.py` (add_to_requisition), `services/requisition_service.py` (clone), `services/quick_source_service.py`. |
| `app/services/offer_service.py` | The ONE offer lifecycle service (W3, spec §9): single implementation of offer create / update / reconfirm / approve / reject / mark-sold / delete plus the reply-scan system-actor create — consolidated from the three drifted router copies (`routers/crm/offers.py` JSON, `routers/htmx/offers/crud.py` HTMX, `routers/sightings.py` wrappers) and the email pipeline's private auto-create. Canonical semantics locked here: reconfirm is TTL-resetting everywhere (expires_at = now+14d, is_stale cleared, attribution back to active); approve lands on `OfferStatus.ACTIVE` (APPROVED retired as a landing state); create composes the richest merged pipeline (vendor-card resolve/fuzzy/create, material card, unavailability release, activity/task/knowledge hooks, spq + qualification stamp + currency persist — the old OPEN→offers requisition write is gone: since W3.3 the `offers` stage is derived from the Offer rows themselves). Authorization stays at the routes. Called by `routers/htmx/offers/crud.py`, `routers/sightings.py`, `routers/crm/offers.py`, `services/ai_offer_service.py`, `email_service.py`. |
| `app/services/buyplan_workflow/buyplan_state.py` | (Inside the `buyplan_workflow` package — flagged here as the W3 state-machine seam.) The single enforced transition table + writer for `BuyPlan.status` (`ALLOWED_TRANSITIONS` + `transition()`, mirroring `requisition_state.py` — which since W3.3 enforces only the 6 stored user-intent values draft/open/hotlist/won/lost/cancelled and DERIVES the mid-pipeline display stages rfqs_sent/offers/quoted from Contact/Offer/QuoteRequisition rows via `derived_status`; migration 209 collapsed the stored 9-state ladder): before W3 the graph lived implicitly in inline guards across the 9 status writers; writers keep their intent-specific pre-checks and route the actual write through `transition()` — if a writer's guard and the table ever drift, the table wins. Writes no ActivityLog row (every buy-plan writer owns its own audit). W3 deleted `BuyPlanStatus.INBOUND` alongside it (migration 208), landing the enum at 6 members. Called by `buyplan_approval.py` (8 lifecycle writers) + `buyplan_lines.py` (`resource_line`'s completed→active backorder reopen). |
| `app/services/pricing_history.py` | One preload query over recent Quotes (sent/won/lost — a quote only counts as a real market price once it left draft) building an MPN/`material_card_id` -> last-quoted-price lookup dict. Seeds the smart default sell price on Build-Quote lines (tab + combined multi-req builder). Called by `app.services.quote_builder_service` + `app.routers.crm._helpers` (re-exported for the Quote-detail and Quote-list pricing-history panels). |
...
| `nightly_tests.sh` | Root-crontab nightly (02:30 UTC) full-quality gate against a CLEAN `origin/main` snapshot in the detached `nightly-main` worktree: (1) `pytest tests/` with coverage (FAIL under 80%; xdist INTERNALERROR classified honestly as CRASH), (2) the self-contained TS Playwright suite (`npm ci` + `npm run build` + `npx playwright test` — `playwright.config.ts` boots its own TESTING/sqlite uvicorn on port 8787, never the live app). The python `tests/e2e/` live-app suite stays EXCLUDED (it drives the LIVE Docker app with a minted admin cookie — live-data hazard; see the script header). Both results fold into one `LAST_STATUS` line; any non-PASS invokes `docker compose exec -T app python -m app.management.notify_nightly_status "<status>"` (Teams Adaptive Card + in-app rows for active admins), guarded so a down docker daemon/container never fails the cron. |
