...
| `verify-backup.sh` | P1.5: scheduled verification that the newest backup is actually restorable — runs `restore.sh --verify` (checksum + `pg_restore --list` sanity check, read-only) against the `LATEST` marker inside the `db-backup` container. Exits nonzero with a clear message on failure so a corrupt backup can't sit unnoticed for the full 30-day retention. Installed as a weekly (Sun 04:00) systemd timer — units in `scripts/systemd/avail-backup-verify.{service,timer}`; install one-liner is in the script's own header comment. |
