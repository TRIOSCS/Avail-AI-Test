...
| status | String 50 | STORED user intent only — 6 values: `draft` -> `open` -> `won`/`lost`; `hotlist` (off-pipeline monitor — see Proactive); `cancelled` (retained). Enforced by `ck_requisitions_status` CHECK, **narrowed by migration 209** (W3.3; was the 9-value ladder from 158): `IN ('draft','open','hotlist','won','lost','cancelled')`. The mid-pipeline Sales Hub stages (`rfqs_sent` -> `offers` -> `quoted`) are **derived, not stored** — `services/requisition_state.derived_status` computes them for stored-`open` rows from data: `QuoteRequisition` membership -> `quoted`, else `Offer.requisition_id` rows -> `offers`, else email `Contact` rows (status != 'failed') -> `rfqs_sent`, else `open`. "open" automatically means sourcing. Legacy `active`/`sourcing`/`reopened` were remapped to `open` and the old `archived` rows to `lost` (158); 209 remapped stored `rfqs_sent`/`offers`/`quoted` rows to `open`. There is **no requisition archive/hide capability** — a requisition ends in `won` or `lost` (each carrying a required `outcome_reason`). `RequisitionStatus` (app/constants.py) is the source of truth (`TERMINAL`={won,lost,cancelled}, `OPEN_PIPELINE`={open} — collapsed by W3.3, `MONITOR`={hotlist}). |
...
| sales_section_reviewed_at | UTCDateTime, nullable | HISTORICAL (W3.7 simplification): the Mark-Reviewed ceremony was dropped — nothing writes or gates on this stamp anymore; section editability is the single lock matrix in `qp_workspace.py` (`QP_SECTION_LOCK_MATRIX`). Kept for pre-W3 history (was: migration 177, stamped by the deleted `toggle_section_reviewed`). |
| purchasing_section_reviewed_at | UTCDateTime, nullable | Ditto — historical only since W3.7. |
| sales_section_reviewed_by_id | FK -> users (SET NULL), nullable | Historical (W3.7): who marked the sales section reviewed, pre-ceremony-drop. |
