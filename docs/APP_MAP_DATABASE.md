...
>
> **Deterministic MPN decode (Phase 1 of MPN→spec enrichment):** `app/services/mpn_decoder/` reads facet specs straight from standard manufacturer drive/DIMM part numbers (Seagate/WD/Toshiba/HGST + Samsung/Hynix/Micron/Kingston/Crucial) — zero network/LLM, strict per-vendor regex gates, unrecognized schemes skipped. The worker second pass (`mpn_decoder/writer.py::decode_and_record_specs`, gated by `settings.mpn_decode_enabled`, default on) writes via `record_spec(source="mpn_decode", confidence=0.95)` BEFORE the AI spec pass, guarded so the decoded commodity matches the card's category. Coverage dry-run: `scripts/decode_mpn_dryrun.py`. OEM/FRU spare numbers don't match the gates → resolved in later phases (PartSurfer cross-ref / datasheet).
