...
>
> **#464 finish (migration 174, 2026-07-02).** The 21 unique constraints the models declared but the migration-built baseline never created now exist for real (all targets duplicate-checked clean on the live PG first), the three model-less legacy tables the old chain created — `buy_plans` (V1, superseded by `buy_plans_v3` via migration 076), `notification_engagement`, `self_heal_log` — are dropped (`IF EXISTS`; live staging already lacked them), and `material_cards.enrichment_status` carries its vocabulary comment on both model and DB. The drift gate's `_GRANDFATHERED_ADD_CONSTRAINTS` / `_GRANDFATHERED_MODIFY_COMMENT` sets are now empty and `_GRANDFATHERED_REMOVE_TABLES` then held `_sp1_desc_backup` (migration 091 downgrade path) and `enrichment_credit_usage` (empty + unreferenced; kept pending an explicit product decision — since resolved and dropped by migration 217; today the set holds `_sp1_desc_backup` and `intel_cache`).
>
> **Dead-schema drop (migration 217, 2026-08-29, Decisions I/J/K/L).** Four independently-scoped drops, all verified dead on prod: (I) the `enrichment_queue` table + `EnrichmentQueue` ORM model — superseded by `material_cards.enrichment_status` as the single enrichment-state source; (J) `excess_lists.total_line_items`, a denormalized write-side counter nothing read (the UI computes line counts live off `len(items)` / a COUNT query); (K) the orphan `sync_logs` and `enrichment_credit_usage` tables (the latter's "kept pending a decision" note above is now resolved — dropped) plus the never-modeled `excess_line_items.market_price` / `demand_score` columns; (L) the 17 legacy NOT-VALID `chk_*` constraints from the 001-era squash (migration 048), superseded by the modern named `ck_*` constraints (migration 212) — `chk_offer_status` excluded, already owned by migration 124. `downgrade()` recreates every dropped object in its historical shape (including `total_line_items`, backfilled exactly from `count(*)`); the only non-reconstructible loss is the `market_price` / `demand_score` cell values on the 2026-03-25 demo seed lists (15 + 15 cells). The drift gate's `_GRANDFATHERED_REMOVE_TABLES` / `_GRANDFATHERED_REMOVE_INDEXES` entries for `enrichment_credit_usage`, `sync_logs`, and their indexes were pruned (dropped-for-real objects don't need grandfathering), and the legacy `chk_*` CHECK-constraint carve-out in `_include_object` was removed — a new undeclared CHECK now fails the gate like everything else.
...
| display_timezone | String 64, nullable | Migration 181. **Per-user DISPLAY timezone** — an IANA zone name (e.g. `America/New_York`, `Asia/Tokyo`) used to render stored-UTC timestamps in this viewer's own zone. Auto-detected from the browser (`Intl.DateTimeFormat().resolvedOptions().timeZone`) and posted to `POST /v2/profile/timezone` on page load when it differs from the stored value; also settable via the Profile-tab `<select>`. NULL ⇒ fall back to the company zone (`COMPANY_TIMEZONE` / `settings.company_timezone`, default `America/New_York`). Read per-request into the `current_user_display_tz_var` contextvar by the async `AuditUserMiddleware` (NOT `require_user` — a ContextVar set in a sync dependency is lost in the threadpool); consumed by the `\|localtime` / `\|localdate` Jinja filters, `template_env._task_due_state`, and `timezones.local_day_sentinel` (task due-date writes). |
...
| sourcing_status | String 20, nullable | open -> sourcing -> offered -> quoted -> won -> lost, plus **hotlist** (migration 210, replaced `archived`): off-pipeline monitor state — the customer uses the part but doesn't need it now; standing demand for the proactive matcher, stays on the sightings board. CHECK `ck_requirements_sourcing_status` whitelists the 7 values; migration 210 remapped archived->lost (sentinel reason "Archived (legacy)" where blank). The parts workspace "Arc" view = `sourcing_status IN (won, lost, hotlist)`. NULLable (client-side default only) — filters must be NULL-safe. |
| outcome_reason | Text, nullable | Migration 185. The per-part "why won / why lost" close reason. Set by the Sales-Hub bulk Won/Lost action (`POST /v2/partials/parts/bulk-outcome`) when a part-line is marked WON/LOST. Nullable at the DB level (existing/non-closed lines stay valid); enforcement is **app-side** — `bulk_outcome` (routers/htmx/parts.py) 400s on a blank reason, then stamps the reason on every selected `Requirement` it transitions to WON/LOST via the sourcing state machine (`transition_requirement`), which since migration 210 also CLEARS the reason on any non-terminal transition (reopen/hotlist). Rendered read-only in the REQ Detail tab. Mirrors the requisition-level `Requisition.outcome_reason` (migration 158). |
| cloned_from_id | FK -> requirements (SET NULL), indexed (`ix_requirements_cloned_from`) | Migration 210. Source part when this requirement was created via Clone-to-Active. Reference only — no relationship(); the PART_CLONED activity rows carry the story. Mirrors `requisitions.cloned_from_id`. |
...
**`ebay_search_queue`** — eBay Browse API queue (migration 219; same structure + same composite-UNIQUE dedup `uq_ebay_queue_requirement_mpn` / lineage columns, same `SearchQueueStatus` vocabulary, ebay_worker as reader/writer). The one behavioral difference: rows never sit in `pending` waiting on an AI commodity gate — the eBay worker has none, so `enqueue_for_ebay_search` writes rows straight to `queued` (via `QueueManager(initial_status=...)`, which still defaults to `pending` for ICS/NC/TBF) and the worker's claim moves them `searching` → `completed`. Nothing in the codebase promotes `pending` → `queued` except the AI gate, so a `pending` eBay row would never be claimable; spend is bounded by a daily Browse API call budget instead of by classification. Backed by the `avail-ebay-worker` host worker (an HTTPS poller, not a browser).
