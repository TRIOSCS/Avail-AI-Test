"""tests/test_worker_priority_lane.py — enrichment-worker priority lane.

Covers: select_batch's leading ORDER pair (enrich_requested_at IS NOT NULL DESC,
enrich_requested_at ASC NULLS LAST) — stamped cards beat the whole backlog, FIFO
among themselves — and run_one_batch clearing the stamp on EVERY batch card
(including not_found outcomes) before the first await.
...
def _mk(db, mpn, *, sc=0, requested=None, status="unenriched", created=NOW, enriched=None):
    card = MaterialCard(
        normalized_mpn=mpn,
