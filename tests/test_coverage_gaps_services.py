...
test_coverage_gaps_services.py — Tests targeting specific uncovered lines in:
  - app/services/performance_service.py (lines 108-109, 274-277, 286-289, 317-318,
    349, 502, 540-545, 648, 892, 902, 912, 922, 932, 946-948, 962-964, 974)
...
class TestPerfDomainAliases:
    """Lines 108-109: domain_aliases loop in compute_vendor_scorecard."""

    def test_scorecard_counts_alias_domains(self, db_session):
    ...
    def test_scorecard_empty_alias_skipped(self, db_session):
        from app.services.performance_service import compute_vendor_scorecard
...
class TestPerfQuoteBuyPlanPreload:
    """Lines 274-277 (Quote offer_ids) and 286-289 (BuyPlan offer_ids)."""

    def test_preloaded_ids_from_quotes_and_buyplans(self, db_session):
        from app.services.performance_service import compute_all_vendor_scorecards

        user = _make_user(db_session, "pre@t.com")
        vc = _make_vendor(db_session, "PreVendor", "prev.com")
        req = _make_requisition(db_session, user.id)
        co = Company(name="PreCo", is_active=True, created_at=datetime.now(timezone.utc))
        db_session.add(co)
        db_session.flush()
        cs = CustomerSite(company_id=co.id, site_name="S1", contact_name="J")
        db_session.add(cs)
        db_session.flush()

        for _ in range(6):
            _make_contact(db_session, req.id, user.id, "PreVendor")

        o1 = _make_offer(db_session, req.id, vc.id, user.id, "PreVendor")
        o2 = _make_offer(db_session, req.id, vc.id, user.id, "PreVendor")
        ...
        assert result["updated"] + result["skipped_cold_start"] >= 1


class TestPerfNullResult:
    """Lines 317-318: compute_vendor_scorecard returns empty -> rollback."""

    def test_null_scorecard_result_handled(self, db_session):
        from app.services.performance_service import compute_all_vendor_scorecards
...
class TestPerfInsufficientData:
    """Line 349: skipped += 1 when is_sufficient_data is False."""

    def test_cold_start_vendor_skipped(self, db_session):
        from app.services.performance_service import compute_all_vendor_scorecards

        user = _make_user(db_session, "cold@t.com")
        vc = _make_vendor(db_session, "ColdV", "coldv.com")
        req = _make_requisition(db_session, user.id)
        _make_contact(db_session, req.id, user.id, "ColdV")
        db_session.commit()
...
class TestPerfDecemberRollover:
    """Line 502: December month_end -> January next year."""

    def test_december_month_end(self, db_session):
        from app.services.performance_service import compute_buyer_leaderboard

        _make_user(db_session, "dec@t.com", role="buyer")
        db_session.commit()
...
class TestPerfBuyplanLineItems:
    """Lines 540-545: BuyPlan line_items loop in compute_buyer_leaderboard."""

    def test_buyplan_offers_extracted(self, db_session):
        from app.services.performance_service import compute_buyer_leaderboard

        user = _make_user(db_session, "bp@t.com", role="buyer")
        vc = _make_vendor(db_session, "BPV", "bpv.com")
        req = _make_requisition(db_session, user.id)
        co = Company(name="BPCo", is_active=True, created_at=datetime.now(timezone.utc))
        db_session.add(co)
        db_session.flush()
        cs = CustomerSite(company_id=co.id, site_name="BPS", contact_name="J")
        db_session.add(cs)
        db_session.flush()
        ...
        result = compute_buyer_leaderboard(db_session, now.date())
        assert result["entries"] >= 1


class TestPerfExistingSnapshot:
    """Line 648: existing BuyerLeaderboardSnapshot is updated."""

    def test_existing_snapshot_updated(self, db_session):
        from app.services.performance_service import compute_buyer_leaderboard

        user = _make_user(db_session, "snap@t.com", role="buyer")
        db_session.commit()

        today = date.today()
        compute_buyer_leaderboard(db_session, today)
        result = compute_buyer_leaderboard(db_session, today)
        assert result["entries"] >= 1

        count = (
            db_session.query(BuyerLeaderboardSnapshot)
            .filter(
                BuyerLeaderboardSnapshot.user_id == user.id,
                BuyerLeaderboardSnapshot.month == today.replace(day=1),
            )
            .count()
        )
        assert count == 1
...
class TestPerfSalespersonAllBatches:
    """Lines 892, 902, 912, 922, 932, 946-948, 962-964, 974."""

    def test_all_metric_batches(self, db_session):
        ...
        result = get_salesperson_scorecard(db_session, date.today())
        assert len(result["entries"]) >= 1

        entry = next(e for e in result["entries"] if e["user_id"] == user.id)
        m = entry["monthly"]
        assert m["new_accounts"] >= 1
        assert m["new_contacts"] >= 1
        assert m["calls_made"] >= 1
        assert m["emails_sent"] >= 1
        assert m["requisitions_entered"] >= 1
        assert m["quotes_sent"] >= 1
        assert m["orders_won"] >= 1
        assert m["won_revenue"] >= 1000.0
        assert m["proactive_sent"] >= 1
        assert m["proactive_converted"] >= 1
        assert m["proactive_revenue"] >= 500.0
        assert m["boms_uploaded"] >= 1
