...
class TestFetchFresh:
    ...
    async def test_all_disabled(self, db_session):
        ...
        assert disabled_count >= 8  # 8+ disabled connectors depending on configuration
    ...
    async def test_no_credentials(self, db_session):
        ...
        assert skipped_count >= 8  # 8+ connectors depending on configuration
    ...
    async def test_no_connectors_returns_early(self, db_session):
        ...
        assert results == []
        assert len(stats) >= 8  # 8+ connectors depending on configuration
