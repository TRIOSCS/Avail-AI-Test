...
class TestBackupScriptExists:
    ...
    def test_script_has_set_euo_pipefail(self, script):
        ...
        assert "set -euo pipefail" in content, (
            f"{script} must use 'set -euo pipefail' for safety"
        )
...
class TestBackupScriptContent:
    ...
    def test_backup_uses_custom_format(self):
        ...
        assert "--format=custom" in content, (
            "backup.sh must use pg_dump custom format (supports parallel restore)"
        )
    ...
    def test_backup_verifies_dump(self):
        ...
        assert "pg_restore --list" in content, (
            "backup.sh must verify dump by listing contents"
        )
    ...
    def test_backup_logs_row_counts(self):
        content = self._read_script("backup.sh")
        assert "users" in content and "companies" in content, (
            "backup.sh must log row counts for critical tables"
        )
...
class TestRestoreScriptContent:
    ...
    def test_restore_requires_confirmation(self):
        ...
        assert "RESTORE" in content and "confirm" in content, (
            "restore.sh must require interactive confirmation"
        )

    def test_restore_creates_safety_backup(self):
        content = self._read_script()
        assert "pre_restore" in content, (
            "restore.sh must create a safety backup before restoring"
        )
    ...
    def test_restore_terminates_connections(self):
        ...
        assert "pg_terminate_backend" in content, (
            "restore.sh must terminate existing connections before DROP"
        )

    def test_restore_post_verification(self):
        content = self._read_script()
        assert "RESTORE COMPLETE" in content, (
            "restore.sh must verify and report after restore"
        )
...
class TestSpacesScriptContent:
    ...
    def test_spaces_skips_when_unconfigured(self):
        content = self._read_script()
        assert "SKIP" in content or "not set" in content, (
            "backup-to-spaces.sh must gracefully skip when not configured"
        )
    ...
    def test_spaces_has_remote_rotation(self):
        content = self._read_script()
        assert "SPACES_RETENTION_DAYS" in content, (
            "backup-to-spaces.sh must support remote rotation"
        )
...
class TestDockerComposeBackup:
    ...
    def test_backup_service_defined(self):
        ...
        assert "db-backup:" in content, (
            "docker-compose.yml must define db-backup service"
        )

    def test_backup_volume_defined(self):
        content = self._read_compose()
        assert "pgbackups:" in content, (
            "docker-compose.yml must define pgbackups volume"
        )
    ...
    def test_backup_depends_on_db(self):
        ...
        assert "db:" in section and "service_healthy" in section, (
            "db-backup must depend on db being healthy"
        )
    ...
    def test_backup_mounts_scripts(self):
        ...
        idx = content.index("db-backup:")
        section = content[idx:idx + 500]
        assert "./scripts:/scripts:ro" in section, (
            "db-backup must mount scripts as read-only"
        )
    ...
    def test_backup_mounts_backup_volume(self):
        ...
        idx = content.index("db-backup:")
        section = content[idx:idx + 500]
        assert "pgbackups:/backups" in section, (
            "db-backup must mount pgbackups volume"
        )
