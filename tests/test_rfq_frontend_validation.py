...
class TestFollowUpPanelRefresh:
    def test_bulk_follow_up_refreshes_panel(self, app_js):
        assert "loadFollowUps" in app_js


class TestRfqErrorToasts:
    def test_retry_rfq_uses_error_toast(self, app_js):
        assert "Retry failed" in app_js

    def test_rfq_send_uses_error_toast(self, app_js):
        assert "Failed to prepare RFQ" in app_js


class TestRfqFollowUpWireup:
    def test_followups_functions_exist(self, app_js):
        assert "sendFollowUp" in app_js
        assert "loadFollowUpsPanel" in app_js


class TestRfqActivityWireup:
    def test_activity_functions_exist(self, app_js):
        assert "loadActivity" in app_js or "activity" in app_js


class TestRfqWorkspaceWireup:
    def test_workspace_functions_exist(self, app_js):
...
class TestRequirementPanelTabs:
    """Verify the requirement detail panel has tabs."""

    def test_rfq_workspace_tab_order(self, app_js):
        """RFQ workspace panel tabs include Offers, Tasks, Notes, History, Sightings."""
        import re
    ...
    def test_rfq_switch_tab_handles_all_tabs(self, app_js):
        """rfqSwitchTab covers offers, sightings, tasks, notes, history."""
        assert "rfqSwitchTab" in app_js

    def test_rfq_render_tab_handles_key_tabs(self, app_js):
        """rfqRender dispatches to key renderers."""
    ...
    def test_tasks_tab_exists(self, app_js):
        """Tasks tab exists in panel."""
        assert "rfqSwitchTab('tasks')" in app_js

    def test_notes_tab_exists(self, app_js):
        """Notes tab exists in panel."""
        assert "rfqSwitchTab('notes')" in app_js
