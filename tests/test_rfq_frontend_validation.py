...
class TestRfqErrorToasts:
    def test_retry_rfq_uses_friendly_error_toast(self, app_js):
        """RFQ retry shows user-friendly error message."""
        assert "retry" in app_js.lower() or "RFQ" in app_js

    def test_update_status_uses_friendly_error_toast(self, app_js):
        """RFQ status update shows user-friendly error message."""
        assert "status" in app_js.lower() or "response" in app_js.lower()
...
class TestRequirementPanelTabs:
    ...
    def test_inline_part_expand_tabs(self, app_js):
        """Inline part expansion has tab handling."""
