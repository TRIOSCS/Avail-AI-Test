...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 5: legacy send endpoint (lines 431-450)
# ═══════════════════════════════════════════════════════════════════════════════


class TestSendLegacy:
    """Cover lines 431-450: legacy per-match send route."""

    def test_legacy_send_match_not_found_returns_404(self, db_session: Session):
        data = _build_scenario(db_session)
        user = data["owner"]
    ...
    def test_legacy_send_no_body_returns_400(self, db_session: Session):
        data = _build_scenario(db_session)
        user = data["owner"]

        for client in _make_client(db_session, user):
            resp = client.post(
                f"/v2/partials/proactive/{data['match'].id}/send",
                data={"body": ""},
                headers=HX,
            )
    ...
    def test_legacy_send_success_marks_sent(self, db_session: Session):
        """Valid body → match.status set to sent, list template returned."""
        data = _build_scenario(db_session)
        user = data["owner"]
        match = data["match"]
        ...
        for client in _make_client(db_session, user):
            resp = client.post(
                ...
                data={"body": "We have BC547 in stock, please let us know!"},
                headers=HX,
            )
    ...
    def test_legacy_send_different_user_match_returns_404(self, db_session: Session):
        """A match belonging to another salesperson returns 404."""
        data = _build_scenario(db_session)

        intruder = User(
            email="intruder@trio.com",
            role="buyer",
            azure_id="az-intruder",
            created_at=datetime.now(timezone.utc),
        )
