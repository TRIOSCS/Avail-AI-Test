...
# ── Line 1149: vendor_contact_nudges tz-naive datetime ───────────────────────


class TestVendorContactNudgesTzNaive:
    """vendor_contact_nudges handles tz-naive last_interaction_at (line 1149)."""

    async def test_nudge_tz_naive_old_interaction(self, db_session: Session, test_vendor_card: VendorCard):
        """Line 1149: tz-naive datetime older than 30 days → contact is nudge candidate."""
        from datetime import timedelta

        from app.routers.htmx.vendors import vendor_contact_nudges

        old_date = datetime.now() - timedelta(days=60)  # tz-naive, old
        vc = VendorContact(
            vendor_card_id=test_vendor_card.id,
            email="dormant@vendor.com",
            full_name="Dormant Contact",
            source="manual",
            is_verified=True,
            confidence=80,
            last_interaction_at=old_date,  # tz-naive triggers line 1149
        )
        db_session.add(vc)
        db_session.commit()
        ...
        with patch("app.routers.htmx.vendors.template_response") as mock_tpl:
            mock_tpl.return_value = HTMLResponse("<div>nudges</div>")
            result = await vendor_contact_nudges(
                request=req,
                vendor_id=test_vendor_card.id,
                user=user,
                db=db_session,
        ...
        nudge_emails = [c.email for c in ctx["nudges"]]
        assert "dormant@vendor.com" in nudge_emails

    async def test_nudge_recent_tz_naive_not_included(self, db_session: Session, test_vendor_card: VendorCard):
        """Tz-naive datetime within 30 days → not a nudge candidate."""
        from datetime import timedelta

        from app.routers.htmx.vendors import vendor_contact_nudges

        recent_date = datetime.now() - timedelta(days=5)  # tz-naive, recent
        vc = VendorContact(
            vendor_card_id=test_vendor_card.id,
        ...
        ctx = mock_tpl.call_args[0][1]
        nudge_emails = [c.email for c in ctx["nudges"]]
        assert "active@vendor.com" not in nudge_emails
