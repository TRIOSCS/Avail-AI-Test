...
class TestLeaderboardSalesBreakdown:
    def test_sales_role_breakdown(self, client, db_session, test_user):
        ...
        snap = MultiplierScoreSnapshot(
            ...
            quotes_won_count=5,
            quotes_sent_pts=20.0,
            quotes_won_pts=30.0,
            proactive_sent_count=8,
            proactive_converted_count=3,
            proactive_sent_pts=16.0,
            proactive_converted_pts=12.0,
...
class TestVendorGetOrCreatePgTrgm:
    ...
    def test_check_vendor_duplicate_fuzzy(self, client, db_session):
        vc = VendorCard(
            normalized_name="alpha semiconductor",
            display_name="Alpha Semiconductor",
            sighting_count=5,
        )
        db_session.add(vc)
        db_session.commit()
...
class TestMaterialMergeVendorHistory:
    def test_merge_with_overlapping_vendor_history(self, db_session, admin_user):
        ...
        target = MaterialCard(
            normalized_mpn="target001",
            display_mpn="TARGET001",
            manufacturer=None,
            description=None,
            search_count=3,
        )
        ...
        source = MaterialCard(
            normalized_mpn="source001",
            display_mpn="SOURCE001",
            manufacturer="TI",
...
class TestSearchServiceBulkCommitFailure:
    def test_save_sightings_with_succeeded_sources(self, db_session):
        ...
        _save_sightings(results, item, db_session, succeeded_sources={"broker_bin"})
        db_session.commit()
    ...
    def test_save_sightings_bulk_failure_retry(self, db_session):
        ...
        user = User(
            email="ss-fail@trioscs.com",
            name="SS Fail",
            role="buyer",
            azure_id="ss-fail-001",
            created_at=datetime.now(timezone.utc),
        )
        db_session.add(user)
        db_session.flush()
...
class TestResolvedMaterialCardPaths:
    ...
    def test_resolve_material_card_race_condition(self, db_session):
        ...
        card = MaterialCard(
            normalized_mpn="racetest001",
            display_mpn="RACETEST001",
            search_count=0,
        )
        db_session.add(card)
        db_session.commit()
        ...
        async def _run():
            with (
                patch(
                    ...
                    return_value=("Test Corp", "test.com"),
                ),
                patch(
                    "app.enrichment_service._explorium_find_company",
                    new_callable=AsyncMock,
                    return_value={"hq_city": "Austin"},
                ),
                patch(
                    "app.enrichment_service._gradient_find_company",
                    new_callable=AsyncMock,
                    return_value={"hq_country": "US"},
                ),
                patch(
                    "app.enrichment_service._ai_find_company",
                    new_callable=AsyncMock,
                    return_value={"industry": "AI Added"},
                ),
                patch("app.enrichment_service.normalize_company_output", side_effect=lambda x: x),
            ):
        ...
        async def _run():
            with (
                patch(
                    "app.enrichment_service.normalize_company_input",
                    new_callable=AsyncMock,
                    return_value=("AI Corp", "ai.com"),
                ),
                patch("app.enrichment_service._explorium_find_company", new_callable=AsyncMock, return_value=None),
                patch("app.enrichment_service._gradient_find_company", new_callable=AsyncMock, return_value=None),
                patch(
                    "app.enrichment_service._ai_find_company",
                    new_callable=AsyncMock,
                    return_value={"industry": "AI Tech"},
                ),
                patch("app.enrichment_service.normalize_company_output", side_effect=lambda x: x),
            ):
                with (
