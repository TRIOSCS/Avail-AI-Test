...
async def test_resolve_uses_flush_not_commit(resolver, db_session, monkeypatch):
    """Caller-owned transaction: resolve() must persist its pending row via
    flush(), never commit() — so the caller keeps ownership of the outer
    transaction. Patch db.commit to blow up; resolve() must still succeed.
    """

    def _boom():
        raise AssertionError("resolve() must not commit the caller's session")
