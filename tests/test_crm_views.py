...
class TestEditSite:
    ...
    def test_post_site_edit_persists_payment_terms_and_address(
        ...
        """POST edit saves payment_terms + address fields; re-rendered sites tab shows new values."""
        from app.models.crm import CustomerSite
    ...
    def test_post_site_edit_re_renders_sites_tab(
        self, client: TestClient, db_session: Session, test_user: User
    ):
        """POST edit response is the refreshed sites tab containing the updated site name."""
        company, site = self._make_company_with_site(db_session)
        resp = client.post(
            f"/v2/partials/customers/{company.id}/sites/{site.id}/edit",
...
class TestEditContact:
    ...
    def test_get_contact_edit_form_returns_200(
        self, client: TestClient, db_session: Session, test_user: User
    ):
        """GET edit-form renders form pre-populated with contact fields."""
        company, site, contact = self._make_company_with_contact(db_session)
        resp = client.get(
            f"/v2/partials/customers/{company.id}/sites/{site.id}/contacts/{contact.id}/edit-form"
        )
        assert resp.status_code == 200
    ...
    def test_get_contact_edit_form_404_on_missing_contact(
        ...
        resp = client.get(
            f"/v2/partials/customers/{company.id}/sites/{site.id}/contacts/99999/edit-form"
        )
        assert resp.status_code == 404

    def test_post_contact_edit_persists_title_and_phone(
        self, client: TestClient, db_session: Session, test_user: User
    ):
    ...
    def test_post_contact_edit_re_renders_contacts_panel(
        ...
        """POST edit response contains the updated name in the re-rendered contacts panel."""
        company, site, contact = self._make_company_with_contact(db_session)
        resp = client.post(
            f"/v2/partials/customers/{company.id}/sites/{site.id}/contacts/{contact.id}/edit",
