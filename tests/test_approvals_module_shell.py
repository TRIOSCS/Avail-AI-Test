...
The Approvals Workspace (Deals / Purchase Orders / Prepayments — W4.3 tabs merge) is
...
def test_buy_plans_full_page_308s_to_workspace(nonadmin_client: TestClient):
    """/v2/buy-plans retired — a 308 onto the workspace Deals tab (spec §11.1)."""
...
def test_buy_plan_detail_url_308s_to_workspace_tab(nonadmin_client: TestClient):
    """Detail deep links land on the workspace's Deals tab CARRYING the plan id —
    ...
    assert r.status_code == 308
    assert r.headers["location"] == "/v2/approvals?tab=deals&select=999999"


# ── Approvals Workspace (3-tab split-view console) ──────────────────────────
...
def test_approvals_page_threads_select(nonadmin_client: TestClient):
    ...
    r = nonadmin_client.get("/v2/approvals?tab=deals&select=42")
    assert r.status_code == 200
    # Jinja autoescapes the & inside hx-get="{{ partial_url }}".
    assert "/v2/partials/approvals?tab=deals&amp;select=42" in r.text
    bad = nonadmin_client.get("/v2/approvals?tab=deals&select=abc")
    assert bad.status_code == 200
    assert 'hx-get="/v2/partials/approvals?tab=deals"' in bad.text
    assert "select=abc" not in bad.text


def test_approvals_shell_renders_three_tabs(nonadmin_client: TestClient):
    """The Approvals Workspace shell renders all three tab URLs + the lazy-body
    guard."""
    r = nonadmin_client.get("/v2/partials/approvals")
    assert r.status_code == 200
    for key in ("deals", "purchase-orders", "prepayments"):
...
def test_buy_plans_lens_partials_308_to_workspace_tab_body(nonadmin_client: TestClient):
    """Both retired lens bodies 308 onto the workspace tab body (the legacy buy-plans
    key — it aliases onto Deals); scope threads through to seed the Mine/All toggle."""
