...
# ── Tests: calendar scan parked (W1) ─────────────────────────────────────────


class TestCalendarScanParked:
    """calendar_scan is PARKED (W1, docs/W1_JOB_DISPOSITION.md): the registration was
    removed because activity_tracking_enabled defaults ON.

    The implementation (_job_calendar_scan) is kept; comeback trigger = team exists
    (spec §5.4/§8).
    """

    def test_calendar_scan_never_registered(self):
        """calendar_scan is not registered regardless of activity_tracking_enabled."""
        ...
        for tracking_enabled in (False, True):
            ...
            mock_settings.activity_tracking_enabled = tracking_enabled
            ...
            assert "calendar_scan" not in all_kwargs_ids, "calendar_scan is parked and must never be registered"
