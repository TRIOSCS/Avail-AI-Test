...
from app.services.excess_mirror import _virtual_req_name
...
def test_delete_companies_tears_down_excess_mirror(db_session):
    """Deleting a company whose excess list carries PRE-EXISTING mirror rows (written
    before the dual-write was retired, SIMPLIFICATION_SPEC §5.3) must DELETE its
    ...
    # Seed the retired mirror's rows directly — the shape the deleted write path left in
    # production DBs: scratch req + requirement + one customer_excess Sighting per line.
    line = db_session.query(ExcessLineItem).filter_by(excess_list_id=el.id).one()
    scratch = Requisition(name=virtual_name, status="open", is_scratch=True, created_by=owner.id)
    db_session.add(scratch)
    db_session.flush()
    requirement = Requirement(requisition_id=scratch.id, sourcing_status="open")
