...
class TestClaimProspect:
    def test_path_b_creates_company_and_site(self, db_session: Session, test_user: User):
        prospect = _make_prospect(
            db_session, domain="newdisco.com", hq_location="Dallas, TX"
        )
    ...
    def test_path_a_updates_existing_company(self, db_session: Session, test_user: User):
        ...
        prospect = _make_prospect(
            db_session, domain="sf-company.com", company_id=company.id
        )
        result = claim_prospect(prospect.id, test_user.id, db_session)
    ...
    def test_path_a_existing_company_same_owner_allowed(
        self, db_session: Session, test_user: User
    ):
        company = _make_company(
            db_session, domain="already-owned.com", owner_id=test_user.id
        )
        prospect = _make_prospect(
            db_session, domain="already-owned.com", company_id=company.id
        )
        result = claim_prospect(prospect.id, test_user.id, db_session)
        assert result["status"] == "claimed"

    def test_path_a_raises_if_company_owned_by_other(
        self, db_session: Session, test_user: User, admin_user: User
    ):
        company = _make_company(
            db_session, domain="other-owned.com", owner_id=admin_user.id
        )
        prospect = _make_prospect(
            db_session, domain="other-owned.com", company_id=company.id
        )
        with pytest.raises(ValueError, match="already owned by another user"):
            claim_prospect(prospect.id, test_user.id, db_session)

    def test_domain_collision_links_to_existing_company(
        self, db_session: Session, test_user: User
    ):
        existing_company = _make_company(
            db_session, name="Existing Corp", domain="collision.com"
        )
        prospect = _make_prospect(
            db_session, domain="collision.com", company_id=None
        )
    ...
    def test_domain_collision_raises_if_other_owner(
        self, db_session: Session, test_user: User, admin_user: User
    ):
        _make_company(
            db_session, domain="owned-collision.com", owner_id=admin_user.id
        )
        prospect = _make_prospect(
            db_session, domain="owned-collision.com", company_id=None
        )
        with pytest.raises(ValueError, match="same domain.*already owned"):
            claim_prospect(prospect.id, test_user.id, db_session)

    def test_raises_lookup_error_if_prospect_not_found(
        self, db_session: Session, test_user: User
    ):
        with pytest.raises(LookupError, match="Prospect not found"):
            claim_prospect(99999, test_user.id, db_session)

    def test_raises_value_error_if_already_claimed(
        self, db_session: Session, test_user: User
    ):
        prospect = _make_prospect(
            db_session,
    ...
    def test_raises_value_error_if_status_not_suggested(
        self, db_session: Session, test_user: User
    ):
        prospect = _make_prospect(
            db_session,
    ...
    def test_raises_value_error_if_site_cap_hit(
        self, db_session: Session, test_user: User
    ):
    ...
    def test_no_domain_on_prospect_skips_collision_check(
        self, db_session: Session, test_user: User
    ):
...
class TestReleaseProspect:
    def test_valid_release_by_owner(self, db_session: Session, test_user: User):
        company = _make_company(
            db_session, domain="release-me.com", owner_id=test_user.id
        )
        prospect = _make_prospect(
            db_session,
    ...
    def test_admin_can_release_others_prospect(
        self, db_session: Session, test_user: User, admin_user: User
    ):
        prospect = _make_prospect(
            db_session,
...
class TestRevealContacts:
    ...
    def test_deduplicates_by_email(self, db_session: Session):
        ...
        site = CustomerSite(
            company_id=company.id, site_name="HQ", is_active=True
        )
        db_session.add(site)
        db_session.flush()
    ...
    def test_creates_site_when_none_exists(self, db_session: Session):
        ...
        prospect = _make_prospect(
            ...
            enrichment_data={
                "contacts_full": [
                    {"name": "Alice", "email": "alice@site-create.com"}
                ]
            },
        )
    ...
    def test_uses_existing_site_when_present(self, db_session: Session):
        ...
        existing_site = CustomerSite(
            company_id=company.id, site_name="Main HQ", is_active=True
        )
        ...
        prospect = _make_prospect(
            ...
            enrichment_data={
                "contacts_full": [
                    {"name": "Bob", "email": "bob@existing-site.com"}
                ]
            },
        )
        reveal_contacts(prospect, db_session)
...
class TestGenerateAccountBriefing:
    ...
    async def test_ai_returns_empty_string_falls_back_to_template(
...
class TestTemplateBriefing:
    ...
    def test_ai_writeup_appended(self, db_session: Session):
        p = _make_db_prospect(
            db_session, "template-writeup.com", ai_writeup="They source a lot of FPGAs."
        )
        result = _template_briefing(p, {}, [])
...
class TestCheckEnrichmentStatus:
    ...
    def test_status_none_when_no_enrichment_data(self, db_session: Session):
        prospect = _make_prospect(
            db_session, domain="no-status.com", enrichment_data={}
        )
...
class TestAddProspectManually:
    def test_new_domain_creates_prospect(self, db_session: Session, test_user: User):
        ...
        prospect = (
            ...
            .filter(ProspectAccount.domain == "acmerobotics.com")
            .first()
        )
    ...
    def test_existing_domain_returns_existing(
        self, db_session: Session, test_user: User
    ):
    ...
    def test_whitespace_only_domain_raises(
        self, db_session: Session, test_user: User
    ):
        with pytest.raises(ValueError, match="Domain is required"):
            add_prospect_manually("   ", test_user.id, db_session)
    ...
    def test_fit_and_readiness_scores_zero(
        self, db_session: Session, test_user: User
    ):
        add_prospect_manually("zero-score.com", test_user.id, db_session)
        prospect = (
            db_session.query(ProspectAccount)
            .filter(ProspectAccount.domain == "zero-score.com")
            .first()
        )
...
class TestTriggerDeepEnrichmentBg:
    ...
    The function calls db.close() in its finally block. We patch SessionLocal to
    return the test session, and also patch db.close() to a no-op so the session
    stays open for post-call assertions.
    """
