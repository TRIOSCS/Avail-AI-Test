...
class TestSeedApiSourcesQuotaBackfill:
    ...
    def test_quota_backfill_sets_value(self):
        """Quota backfill uses existing_map to set monthly_quota on known sources."""
        ...
        with patch("app.database.SessionLocal") as mock_session_cls:
            ...
            # Simulate hunter_enrichment source with no quota in existing_map
            mock_src = MagicMock()
            mock_src.name = "hunter_enrichment"
            mock_src.monthly_quota = None

            # existing_map is built from .all()
            mock_db.query.return_value.all.return_value = [mock_src]

            _seed_api_sources()

            # The quota should have been set via existing_map lookup
