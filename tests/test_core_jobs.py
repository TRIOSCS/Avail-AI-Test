...
class TestJobBatchEnrichMaterials:
    """Tests for _job_batch_enrich_materials()."""

    @pytest.mark.asyncio
    async def test_submits_batch(self):
        """Successful batch enrich returns batch_id."""
        from app.jobs.core_jobs import _job_batch_enrich_materials

        mock_db = MagicMock()
        mock_enrich = AsyncMock(return_value="batch-123")

        with (
            patch("app.jobs.core_jobs.SessionLocal", return_value=mock_db),
            patch("app.services.material_enrichment_service.batch_enrich_materials", mock_enrich),
            patch("asyncio.wait_for", mock_enrich),
        ):
            await _job_batch_enrich_materials.__wrapped__()

        mock_db.close.assert_called_once()

    @pytest.mark.asyncio
    async def test_handles_timeout(self):
        """TimeoutError causes rollback and re-raise."""
        from app.jobs.core_jobs import _job_batch_enrich_materials
        ...
        with (
            patch("app.jobs.core_jobs.SessionLocal", return_value=mock_db),
            patch("app.services.material_enrichment_service.batch_enrich_materials", AsyncMock()),
            patch("asyncio.wait_for", side_effect=asyncio.TimeoutError()),
        ):
            with pytest.raises(asyncio.TimeoutError):
                await _job_batch_enrich_materials.__wrapped__()
    ...
    async def test_handles_exception(self):
        """Generic exception causes rollback and re-raise."""
        from app.jobs.core_jobs import _job_batch_enrich_materials
        ...
        with (
            patch("app.jobs.core_jobs.SessionLocal", return_value=mock_db),
            patch("app.services.material_enrichment_service.batch_enrich_materials", AsyncMock()),
            patch("asyncio.wait_for", side_effect=RuntimeError("enrich failed")),
        ):
...
class TestJobPollMaterialBatch:
    """Tests for _job_poll_material_batch()."""

    @pytest.mark.asyncio
    async def test_processes_results(self):
        """Successful poll returns applied/errors counts."""
        from app.jobs.core_jobs import _job_poll_material_batch

        mock_db = MagicMock()
        mock_poll = AsyncMock(return_value={"applied": 3, "errors": 1})

        with (
            patch("app.jobs.core_jobs.SessionLocal", return_value=mock_db),
            patch("app.services.material_enrichment_service.process_material_batch_results", mock_poll),
            patch("asyncio.wait_for", mock_poll),
        ):
            await _job_poll_material_batch.__wrapped__()

        mock_db.close.assert_called_once()

    @pytest.mark.asyncio
    async def test_handles_none_result(self):
        """None result (no batch pending) doesn't error."""
        from app.jobs.core_jobs import _job_poll_material_batch

        mock_db = MagicMock()
        mock_poll = AsyncMock(return_value=None)
    ...
    async def test_handles_timeout(self):
        ...
        with (
            patch("app.jobs.core_jobs.SessionLocal", return_value=mock_db),
            patch("app.services.material_enrichment_service.process_material_batch_results", AsyncMock()),
            patch("asyncio.wait_for", side_effect=asyncio.TimeoutError()),
        ):
            with pytest.raises(asyncio.TimeoutError):
                await _job_poll_material_batch.__wrapped__()
