...
Covers: all 3 API functions, graceful no-key fallback, error handling, edge cases.
"""
            ...
            "contacts": [
                {
                    ...
                    "emailAddresses": [{"email": "alice@acme.com", "type": "work"}],
                    "phoneNumbers": [{"number": "+1-555-0200", "type": "mobile"}],
                    "confidence": 85,
                },
                {
                    "firstName": "Bob",
                    ...
                    "emailAddresses": [{"email": "bob@acme.com", "type": "work"}],
                    "phoneNumbers": [],
                    "confidence": 70,
                },
            ],
...
@pytest.mark.asyncio
async def test_search_contacts_api_error(_mock_http):
    _mock_http.get = AsyncMock(return_value=MagicMock(status_code=500, text="Server error"))
    from app.connectors.lusha_client import search_contacts
...
async def test_search_contacts_exception(_mock_http):
    _mock_http.get = AsyncMock(side_effect=Exception("Timeout"))
    from app.connectors.lusha_client import search_contacts
    result = await search_contacts("acme.com")
    assert result == []
...
def test_best_email_priority():
    ...
    assert _best_email(emails) == "work@acme.com"
