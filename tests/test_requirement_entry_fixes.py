...
# ── Frontend regression: duplicate class attribute ───────────────────


def test_contact_field_no_duplicate_class():
    """index.html nrContactField div should have a single class attribute with u-hidden."""
    with open("app/templates/index.html") as f:
        html = f.read()
    # Find the nrContactField div
    import re

    matches = re.findall(r'<div[^>]*id="nrContactField"[^>]*>', html)
    assert len(matches) == 1
    div = matches[0]
    # Should NOT have two class attributes
    class_count = div.count("class=")
    assert class_count == 1, f"nrContactField has {class_count} class attributes: {div}"
    assert "u-hidden" in div


# ── Frontend regression: add-row column count ────────────────────────


def test_add_row_has_expand_arrow_column():
    """_appendAddRow creates an expand-arrow td so columns align with header."""
    with open("app/static/app.js") as f:
        js = f.read()
    # The function should have an expand-arrow comment before badge
    fn_start = js.find("function _appendAddRow")
    assert fn_start > 0
    fn_end = js.find("\nfunction ", fn_start + 1)
    fn_body = js[fn_start:fn_end]
    assert "Expand arrow" in fn_body, "Missing expand arrow td in _appendAddRow"
    # Expand arrow should come before Badge
    expand_pos = fn_body.find("Expand arrow")
    badge_pos = fn_body.find("Badge")
    assert expand_pos < badge_pos, "Expand arrow td must come before Badge td"


# ── Frontend regression: deadline re-enabled after ASAP ──────────────


def test_deadline_disabled_reset_on_clear():
    """_clearNrValidation resets nrDeadline disabled state."""
    with open("app/static/app.js") as f:
        js = f.read()
    fn_start = js.find("function _clearNrValidation()")
    assert fn_start > 0
    fn_end = js.find("\nfunction ", fn_start + 1)
    fn_body = js[fn_start:fn_end]
    assert "'nrDeadline', 'disabled', false" in fn_body


# ── Frontend regression: double-submit guard on createRequisition ────


def test_create_requisition_has_double_submit_guard():
    """createRequisition has a pending flag to prevent double-submit."""
    with open("app/static/app.js") as f:
        js = f.read()
    assert "_createReqPending" in js
    fn_start = js.find("async function createRequisition()")
    assert fn_start > 0
    fn_end = js.find("\nfunction ", fn_start + 1)
    fn_body = js[fn_start:fn_end]
    assert "_createReqPending" in fn_body


# ── Frontend regression: inline edit prevents blank MPN ──────────────


def test_inline_edit_prevents_blank_mpn():
    """editDrillCell blocks saving an empty primary_mpn."""
    with open("app/static/app.js") as f:
        js = f.read()
    fn_start = js.find("function editDrillCell(")
    assert fn_start > 0
    fn_end = js.find("\nfunction ", fn_start + 1)
    fn_body = js[fn_start:fn_end]
