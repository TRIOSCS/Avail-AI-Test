"""test_sightings_batch_ops_coverage.py — Coverage gap tests for sightings batch
endpoints.
...
def test_batch_assign_multiple_requirements_pluralizes_message(client, db_session, test_user):
    """Batch-assign with 2 requirements uses plural 'requirements' in toast (line
    781)."""
...
def test_batch_status_skip_invalid_transition(client, db_session, test_user):
    """Requirements that can't transition to target status are skipped (lines
    831-832)."""
...
def test_mark_unavailable_no_matching_sightings_still_succeeds(client, db_session, test_user):
    """Mark-unavailable with no matching sightings commits and calls
    sightings_detail."""
...
def test_send_inquiry_success_toast_with_progressed_count(client, db_session, test_user):
    """Send-inquiry toast mentions progressed count when auto-progress fires (line
    1290-1291)."""
...
def test_send_inquiry_unknown_vendor_empty_email(client, db_session, test_user):
    """Send-inquiry with vendor not in DB still sends (vendor_email stays empty, line
    1223)."""
