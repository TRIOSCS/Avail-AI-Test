...
def test_better_price_alert_fires(db_session, test_user):
    ...
    asyncio.get_event_loop().run_until_complete(_fire_deal_condition_alerts(db_session, new_offer, req, mock_send))
...
def test_no_alert_on_first_offer(db_session, test_user):
    ...
    mock_send = AsyncMock(return_value=True)
    asyncio.get_event_loop().run_until_complete(_fire_deal_condition_alerts(db_session, first_offer, req, mock_send))
...
def test_qty_filled_alert_fires_on_threshold(db_session, test_user):
    ...
    req, requirement, existing = _setup_offer_scenario(db_session, test_user, existing_price=1.00, target_qty=1000)
...
def test_no_qty_alert_when_already_above(db_session, test_user):
    ...
    req, requirement, existing = _setup_offer_scenario(db_session, test_user, existing_price=1.00, target_qty=100)
...
def test_no_alert_without_target_qty(db_session, test_user):
    ...
    req, requirement, _ = _setup_offer_scenario(db_session, test_user, with_existing_offer=False, target_qty=0)
