...
def _ensure_gradient_api_key():
    ...
    yield
    settings.do_gradient_api_key = original
...
def _chat_response(content, model="anthropic-claude-4.6-sonnet", prompt_tokens=50, completion_tokens=20):
...
def test_model_tiers():
    """Verify model tier mapping includes expected keys and strong tier."""
...
async def test_gradient_text_default_model():
    """Default model tier uses the configured default model."""
    mock_data = _chat_response("result", model=MODELS["default"])
    with patch("app.services.gradient_service.http") as mock_http:
        mock_http.post = AsyncMock(return_value=_mock_response(200, mock_data))
    ...
    call_args = mock_http.post.call_args
    body = call_args.kwargs.get("json") or call_args[1].get("json")
    assert body["model"] == MODELS["default"]
